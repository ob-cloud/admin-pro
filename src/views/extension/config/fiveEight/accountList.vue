<template>
  <div class="accountList-box">
    <div class="basicsInfo">
      <div class="table">
        <div class="table-title">
          ...
          <div style="max-width: 25%;text-align: left;padding-left: 21px;background: #F5F7FF;font-size:14px;color: #666666;">clientId</div>
          <div style="max-width: 31%;text-align: left;padding-left: 21px;background: #F5F7FF;font-size:14px;color: #666666;">clientSecret(密钥)</div>
        ...
        <div class="house-data u-no-border" v-for="(item,index) in accoutList" :key="index">
          ...
          <div class="overHidde cursor" @click="openAccountDetail(item)" style="max-width: 31%;text-align: left;padding-left: 21px;">{{item.clientSecret}}</div>
        ...
        <div class="house-data u-no-border cursor" @click="Open58Dialog=true;" v-if="accoutList.length==0">
          <div class="continueAddStyle">
            <icon-font type="iconyumengtubiao_tianjia" />绑定账号
          </div>
        </div>
        <div class="house-data u-no-border cursor" v-mark="['mk_extension_config_bdzh']" @click="Open58Dialog=true;" v-if="accoutList.length>0">
          <div class="continueAddStyle">
    ...
    <!-- 账号详情 -->
    <a-modal destroyOnClose centered :footer="null" :maskClosable="false" width="520px" v-model="accountDetailDialog" class="ownModalBorder titDialog" title="58租房账号详情">
...
export default {
  ...
  methods:{
    ...
    openAgentList(item){
      ...
      this.agentListTitle = '管理58租房"'+ item.cityName +'"管家信息';
      this.clientKeyId = item.id;
    },
    deleteAccount(item){
      let that = this;
      this.$modal.confirm({
        title: '确定要解除"' + item.cityName + '"绑定的58租房账号吗？',
        content: '解绑后，该账号下已上架所有房源将同步从58租房平台上下架；且房源将无法在推广到58租房平台上',
        width:"433px",
        height:"192px",
        okText: '确认',
        cancelText: '取消',
