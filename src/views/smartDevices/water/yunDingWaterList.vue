<template>
  <div class="yunDingWaterList-box">
        ...
        <div class="basicsInfo" :style="tableData.length==0?'padding-bottom: 20px;':''">
          <div class="table">
            <div class="table-title">
              ...
              <div style="max-width: 302px;text-align: left;padding-left: 9px;">水表序列号</div>
            ...
            <div class="house-data u-no-border" v-for="(item) in tableData" :key="item.houseId">
              <div class="item" style="max-width:299px;text-align: left;padding-left: 20px;display: flex;align-items: center;" :style="item.roomList.length==1||item.roomList.length==0?'height:60px;line-height:60px;':'height:'+item.roomList.length*40+'px;'">
                 <div :style="item.roomList.length==0?'height:60px;line-height:20px;margin-top:18px;':''">
              ...
              <div class="item" v-else style="max-width: 658px;" :style="item.roomList.length==1?'height:60px;line-height:60px;':''">
                <div class="itemDetail" v-for="(temp,index) in item.roomList" :class="index==item.roomList.length-1?'itemDetailLast':''">
                  ...
                  <div style="max-width: 98px;text-align: center;background: #F7F8FB;font-size: 14px;color: #BBBBBB;" v-if="!temp.isHaveWaterMeter||temp.isHaveWaterMeter==0">
                    无电表
                  </div>
                  <div style="max-width: 98px;text-align: center;background: #F7F8FB;font-size: 14px;color: #BBBBBB;" v-if="temp.isHaveWaterMeter&&temp.isHaveWaterMeter==1&&temp.isBound==1">
                    已绑定
                  </div>
                  <div style="max-width: 98px;text-align: center;background: #F7F8FB;font-size: 14px;color: #BBBBBB;" v-if="temp.isHaveWaterMeter&&temp.isHaveWaterMeter==1&&temp.isBound==0&&selectedWaterIds.indexOf(temp.uuid)>-1&&houseObj.waterId!=temp.uuid">
                    已选择
                  </div>
                  <div class="checkStyle cursor" @click="selectedWaterUuidFn(item,temp)" style="max-width: 98px;text-align: center;" v-if="temp.isHaveWaterMeter&&temp.isHaveWaterMeter==1&&temp.isBound==0&&(selectedWaterIds.indexOf(temp.uuid)==-1||houseObj.waterId==temp.uuid)">
  ...
  created() {
    if(this.houseObj){
      ...
      if(this.houseObj.waterId){
        this.selectedUuid = this.houseObj.waterId;
      }
      this.sendData.waterMeterType = this.houseObj.waterMeterType;
    }
    this.waterList();
  },
  methods: {
    selectedWaterUuidFn(item,temp){
      ...
      this.selectedObj.uuid = temp.uuid;
    ...
    waterList(){
      API.listPullYundingWaterMeter(this.sendData).then(res => {
        if (res&&res.code === "200") {
            ...
            this.tableData.forEach((item)=>{
              item.roomList.forEach((temp)=>{
                if(this.selectedUuid == temp.uuid){
                  this.selectedWaterUuidFn(item,temp);
                }
              });
            });
          }
