        ...
        <div class="ipt-box">
            <label-item title="房源信息" border :mustFill="true">
                <div class="u-bottom-border address">
                    <a-select showSearch :value="parData.fullAddress" placeholder="请输入房源编号/小区地址/项目名称/门牌号/业主姓名/业主电话检索房源" :class="{ 'must-fill-border': mustFillBorder  }" style="width: 100%;" :defaultActiveFirstOption="false" :showArrow="false" :filterOption="false" @search="selectCommunity" @blur="isEmpty">
    ...
    data() {
        return {
            ...
            parData: {
                bizId: '',
                id: '',              //预定ID
                relationType:3,  //关联类型(1：房源；2：租客合同；3：业主合同)
    ...
    methods: {
        ...
        save() {
            if (!this.parData.bizId) {
                this.mustFillBorder = true;
                this.$message.warning('请选择要关联的业主！');
                return;
            }
