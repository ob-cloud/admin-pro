            ...
            <div>
                <div class="search-condition clearfix">
                    ...
                    <div class="f-fl clearfix" style="margin-top: 7px">
                        ...
                        <div class="f-fl" style="margin-top: 0.5px;"><dept-employee-tree placeholder="请选择部门" resMark="mk_contract_owner" :showNameAll="false" :showEmployee="false" @selectChange="selectDept" placement="bottomRight" ref="selectDeptEmp" minWidth="250px" width="90px" ></dept-employee-tree></div>
                    </div>
                    <div class="search-condition-divider f-fl" style="width: 1px"></div>
        ...
        <div class="con" v-show="statusActiveIndex == 1">
            <a-table :loading="loading" size="middle" rowKey="id" :columns="columns" :dataSource="tableData.list" :pagination="false" bordered>
                <div slot="status" slot-scope="text">
                    <span v-if="text==1" class="status1">待生效</span>
                    <span v-else-if="text==2" class="status2">合约中</span>
                    <span v-else-if="text==3" class="status4">即将到期</span>
                    <span v-else-if="text==4" class="status5">已到期</span>
                    <span v-else-if="text==5" class="status6">正常退</span>
                    <span v-else-if="text==6" class="status7">非正常退</span>
                    <span v-else-if="text==7" class="status8">已作废</span>
                    <span v-else-if="text==8" class="status9">已被续约</span>
                </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                exportUrl: '/pc2b/da/owner_contract/list_export',
                exportData:{},
        ...
        methods: {
            ...
            initData1(){
                API.ownerContractDataStat(this.sendData).then(res => {
                    if (res.code === '200') {
                        this.data1 = res.data;
                    }
                })
            ...
            initChart12Data(val){
                API.ownerContractDataAnalysis(this.sendData).then(res => {
                    if (res.code === '200') {
                        let temp = res.data;
                        this.chart1Data = [
                            {
            ...
            initChart3Data(val){
                API.ownerContractListPeriodStat(this.sendData).then(res => {
                    if (res.code === '200') {
            ...
            initChart45Data(val){
                ...
                this.chart4Data = [];
                this.chart5Data = [];
                API.ownerContractListDateDataStat(this.sendData).then(res => {
                    if (res.code === '200') {
