<template>
  <div class="chaoyiEletricList-box">
        ...
        <div class="basicsInfo" :style="tableData.length==0?'padding-bottom: 20px;':''">
          <div class="table">
            ...
            <div class="house-data u-no-border" v-for="(item) in tableData" :key="item.houseId" >
              ...
              <div class="item" style="max-width: 98px;text-align: center;background: #F7F8FB;font-size: 14px;color: #BBBBBB;" v-if="selectedElectricIds.indexOf(item.deviceId)>-1&&houseObj.electricId!=item.deviceId">
                已选择
              </div>
              <div class="checkStyle item cursor" @click="selectedEletricUuidFn(item)" style="max-width: 98px;text-align: center;" v-if="(selectedElectricIds.indexOf(item.deviceId)==-1||houseObj.electricId==item.deviceId)">
                <icon-font class="selectStyle" v-if="selectedEletricUuid==item.deviceId" type="iconyumengtubiao_danxuan-yixuanze" />
...
export default {
  ...
  created() {
    if(this.houseObj){
      ...
      this.eletricIndex = this.houseObj.eletricIndex;
      if(this.houseObj.electricId){
        this.selectedEletricUuid = this.houseObj.electricId;
      }
    }
    this.waterList();
  },
  methods: {
    ...
    selectedEletricUuidFn(item){
      ...
      this.selectedEletricUuid = item.deviceId;
    },
    cancelSelect(){
    ...
    confirmSelect(){
      ...
      this.$emit('confirmSelect',this.selectedObj,this.index,this.eletricIndex)
    },
    ...
    waterList(){
      ...
      API.listPullJoyElectricMeter(this.sendData).then(res => {
        if (res&&res.code === "200") {
          this.tableData = res.data.records;
          this.pagination.total = res.data.total;
          if(this.selectedEletricUuid){
            this.tableData.forEach((item)=>{
              if(this.selectedEletricUuid == item.deviceId){
                this.selectedEletricUuidFn(item);
              }
            })
          }
        }
