  ...
  <div class="chargeAmount-box">
    <div class="headInfo margin-top10">
      <div class="clearfix" style="padding-bottom: 12px;border-bottom: 1px solid #E8E8E8;">
        ...
        <div class="f-fl">
          <div class="doorplateStyle">{{waterData.doorplate}} - {{waterData.name}}</div>
          <div class="fullAddressStyle">{{waterData.fullAddress}}</div>
        </div>
      </div>
      ...
      <div class="margin-top10">
        ...
        <div class="item item-content">{{waterData.unitPrice|blankVal}} 元/度</div>
      </div>
      <div class="margin-top10">
        <div class="item width1">剩余水量：</div>
        <div class="item item-content">{{ (waterData.totalCharge-waterData.powerUsed).toFixed(2)}} 度</div>
      </div>
    </div>
    <div class="detail">
        ...
        <label-item class="u-bottom-border" title="充值电量" :mustFill="true">
          <div class="clearfix" style="margin-top: 6px;">
            <div class="f-fl clearfix" style="border-bottom: 1px solid #dddddd;width: 200px;">
              ...
              <div class="f-fl chargeTypeStyleCom chargeTypeStyle2 cursor" :class="chargeType==2?'clickType':'unClickStyle'" @click="getValue(2)">按水量充</div>
  ...
  methods: {
    ...
    setChargeAmount(){
      ...
      if(this.isGenBill){
        ...
        API.waterMeterChargeAmount(this.sendData).then(res => {
          if(!res){
    ...
    viewWater(){
      API.waterMeterView({id:this.waterId}).then(res => {
        if(res.code === '200'){
          ...
          this.houseObj.fullAddress = this.waterData.cityName+ this.waterData.areaName + this.waterData.fullAddress;
          this.houseObj.houseAlias = this.waterData.houseAlias;
          this.houseObj.houseId = this.waterData.houseId;
          this.houseObj.renterName = this.waterData.renterName;
          this.houseObj.renterPhone = this.waterData.renterPhone;
          this.houseObj.renterPaymentType = this.waterData.renterPaymentType;
          this.houseObj.renterPaymentNo = this.waterData.renterPaymentNo;
          this.houseObj.renterBank = this.waterData.renterBank;
        }
      });
    },
  },
