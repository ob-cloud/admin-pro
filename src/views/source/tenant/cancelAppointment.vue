<template>
    <div class="cancel-repair">
        <div class="title">
            确定要取消客户「{{infoData.name}}-{{infoData.phone}}」的预约信息吗？
        </div>
...
export default {
    ...
    methods: {
        save() {
            ...
            API.reservationCancel(parameter).then(res => {
                if (res.code === "200") {
                ...
                this.$emit('successCancelAppointment');
            })

        },
        cancelSave() {
            this.$emit('cancelCancelAppointment');
        }
    },
    filters: {
    },
