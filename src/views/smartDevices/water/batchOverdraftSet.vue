  ...
  <div class="batchOverdraftSet-box">
    ...
    <div class="detail">
      ...
      <div style="color: #FFA036;font-size: 13px;margin-top: 12px;">
        <a-icon style="margin-right: 5px;" type="exclamation-circle" />如果修改的房间内有多个水表，则其他水表都将设置为统一的透支额度
      </div>
    ...
    <a-modal destroyOnClose centered :footer="null" :maskClosable="false" width="520px" v-model="batchSetOverdraftDialog" class="ownModalBorder titDialog" title="修改透支额度" @cancel="closeThis">
      <batch-overdraft-set-result :selectedRowKeys="selectedRowKeys" :coldWaterCheckedCount="coldWaterCheckedCount" :hoteWaterCheckedCount="hoteWaterCheckedCount" @closeThis="closeThis"></batch-overdraft-set-result>
    </a-modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    batchSetOverdraft(){
      ...
      API.waterMeterOverdraftBatchSet(this.sendData).then(res=>{
        if(!res){
