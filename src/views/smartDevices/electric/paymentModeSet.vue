    ...
    viewElectric(){
      API.electricMeterView({id:this.electricMeterId}).then(res => {
        if(res.code === '200'){
          ...
          //初始化付费模式及透支额度
          if(this.electricData.paymentMode){
            this.sendData.paymentMode = this.electricData.paymentMode;
          }
          if(this.electricData.overdraft){
            this.sendData.overdraft = this.electricData.overdraft;
          }
        }
      });
    },
