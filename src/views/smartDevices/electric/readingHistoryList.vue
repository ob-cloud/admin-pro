    ...
    <div class="table">
      <a-table :locale="tablenodata" rowKey="id" :bordered="true" :pagination="false" :columns="tableColumns" :dataSource="tableData">
        ...
        <div slot="readValue" slot-scope="text">
          <span v-if="readingObj.billingMethod==1">余数：{{text?text+'度':'-'}}</span>
          <span v-if="readingObj.billingMethod==2">底数：{{text?text+'度':'-'}}</span>
...
export default {
  ...
  methods: {
    checkOpts(record){
      ...
      let markArr = ['mk_smartDevices_electric_cbjlsc','mk_smartDevices_electric_cbsczd'];
      let btnArr = [];
      //判断资源权限
      markArr.forEach(ele => {
        permissions.some(function(value){
          if (ele == value.mark){
            if(value.mark == 'mk_smartDevices_electric_cbjlsc'){
              btnArr.push('1');
            }
            else if(value.mark == 'mk_smartDevices_electric_cbsczd'&&record.isGenBill==0&&record.totalFee>0){
              btnArr.push('2');
            }
            return true;
          }
    ...
    remove(record){
      ...
      this.$modal.confirm({
        ...
        cancelText: '取消',
        onOk() {
          API.readRecordRemove({id:id}).then(res => {
            if (res.code == 200) {
    ...
    initData(){
      ...
      API.readRecordList(this.sendData).then(res => {
        if(res.code === '200'){
