        ...
        <div class="ipt-box">
            <label-item title="房源信息" border :mustFill="true">
                <div class="u-bottom-border address">
                    <a-select showSearch :value="parData.fullAddress" placeholder="请输入房源编号/小区地址/项目名称/门牌号/租客姓名/租客电话/业主姓名/业主电话检索房源" :class="{ 'must-fill-border': mustFillBorder  }" style="width: 100%;" :defaultActiveFirstOption="false" :showArrow="false" :filterOption="false" @search="selectCommunity" @blur="isEmpty" >
...
import { bizRelation } from '@/api/bill'
...
export default {
    ...
    data() {
        return {
            ...
            parData: {
                houseId: '',
                id: '',              //预定ID
                relationType:1,  //关联类型(1：房源；2：租客合同；3：业主合同)
    ...
    methods: {
        ...
        getCommunity(res) {
            this.parData.houseId = res.houseId;
