    ...
    <div id="addWholeHouse" class="add-share-house">
        ...
        <label-item title="小区地址" border>
            <div class="u-bottom-border address">
                <a-select style="min-width: 100px" v-model="houseData.cityId" @change="selectCity" :showSearch="true" :filterOption="(inputValue,option)=>filterOption(inputValue,option)"
                          >
                    <a-select-option :value="item.id" :key="item.id" v-for="(item) in cityList">{{ item.name }}
                    </a-select-option>
                </a-select>
                <a-select
                        showSearch
        ...
        <label-item title="小区信息" border>
            <div class="village-info u-bottom-border">
                ...
                <div style="margin-top: 28px">
                    <a-checkbox v-model="houseData.isHaveElevator ? true : false" @change="onChangeElevator">有电梯
                    </a-checkbox>
            ...
            <div class="house-list">
                <div class="house-item">
                    <div class="house-detail">
                        ...
                        <div class="house-data u-no-border " v-for="(item,index) in houseData.houseWholeInfoAddFormList" :key="index">
                            ...
                            <div style="max-width: 84px" :class="{'select-border': houseSelectBorder==index && childSelectBorder == -1 && selectBorder=='orientation'}">
                                <a-select style="width: 100%;padding-left: 10px;box-sizing: border-box" v-model="item.orientation" @dropdownVisibleChange="changeSelect(index,-1,'orientation')" >
                            ...
                            <div style="position: relative;max-width: 60px">
                                <a-input v-mustFill:[clickSave] v-model="item.houseArea" v-validate="'area'" placeholder="必填" /><span style="position: absolute;right: 2px">㎡</span>
                            </div>
                            <div style="max-width: 100px" :class="{'select-border': houseSelectBorder==index && childSelectBorder == -1 && selectBorder=='decorationType'}">
                                <a-select style="width: 100%;padding-left: 10px;box-sizing: border-box"  v-model="item.decorationType" @dropdownVisibleChange="changeSelect(index,-1,'decorationType')" >
                          ...
                          <div @click="showMoreModal(index)">
                            ...
                            <div style="max-width: 40px">
                                <icon-font @click="deleteHouse(index)" class="delete-icon" type="iconyumengtubiao_shanchu-"></icon-font>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <a-modal centered cancelText="取消" okText="保存" :maskClosable="false" width="1000px" v-model="fastSetDialog"  @ok="saveFastSet" class="ownModalBorder titDialog" title="快捷设置房源信息">
            <whole-house-fast-set ref="fastSet" :housePublicConfig="housePublicConfig" :configRelationFlag="configRelationFlag" :fastSetData="fastSetData"></whole-house-fast-set>
        </a-modal>
        ...
        <a-modal title="编辑房源标题/描述" centered v-model='showTitleDialog' class="ownModalBorder titDialog" @ok="saveTitle" okText="确认" cancelText="取消">
            ...
            <label-item style="margin: 0 20px 20px 20px" title="房源描述" leftLine border class="u-bottom-border" :valLength="textAreaVal_desc.length" :maxSize="500" >
              <a-textarea :autosize="{ minRows: 1, maxRows: 5 }" v-model="textAreaVal_desc" maxlength="500" placeholder="请输入" />
            </label-item>
        </a-modal>
        <!-- 房源更多信息 -->
        <a-modal title="更多信息" centered v-model='showMoreDialog' class="ownModalBorder titDialog" style="position: relative" @ok="saveShowMore" okText="确认" cancelText="取消">
          <div class="moreStyle">
            <label-item style="margin: 0 20px 20px 20px" title="VR看房地址" border class="u-bottom-border" :valLength="currentMoreInfo.vrUrl.length" :maxSize="100">
              <a-textarea autosize v-model="currentMoreInfo.vrUrl" maxlength="100" placeholder="请输入" />
            </label-item>
            <label-item style="margin: 0 20px 20px 20px" title="房源核验码" border class="u-bottom-border" :valLength="currentMoreInfo.checkCode.length" :maxSize="30">
        ...
        <!-- 物品配置-不关联资产 -->
        <a-modal title="编辑房源配置" destroyOnClose centered :maskClosable="false" :footer="null" width="800px"
                 v-model="editConfigDialog1" class="ownModalBorder titDialog" @close="editConfigDialog1=false">
            <config-default :fromType="fromType" :housePublicConfig="housePublicConfig" :currentConfig="currentConfig" @cancelSaveRoomConfig="cancelSaveRoomConfig" @saveRoomConfig="saveRoomConfig"></config-default>
        </a-modal>
        <!-- 物品配置-关联资产 -->
        <a-modal title="编辑房源配置" destroyOnClose centered :maskClosable="false" :footer="null" width="1000px"
                 v-model="editConfigDialog2" class="ownModalBorder titDialog" @close="editConfigDialog2=false">
          <config-asset :fromType="fromType" :housePublicConfig="housePublicConfig" :currentConfig="currentConfig" @cancelSaveRoomConfig="cancelSaveRoomConfig" @saveRoomConfig="saveRoomConfig"></config-asset>
        </a-modal>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            this.listHouseLable();
            let obj = this.houseData.houseWholeInfoAddFormList[0];
        ...
        methods: {
          ...
          listHouseLable(){
            ...
            savePic(){
                ...
                let houseIndex = this.currentSelectHouseIndex;
                let list = this.houseData.houseWholeInfoAddFormList[houseIndex];
