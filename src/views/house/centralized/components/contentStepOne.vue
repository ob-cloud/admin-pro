      ...
      <div class="house-model f-clearfix">
		...
		<div class="house_info_css">
			...
			<input-item title="楼层" class="lastItem" :must="true" :required="requeredData.floorInfo.required" :error="requeredData.floorInfo.error">
				<template slot="content">
					<a-input placeholder="请输入" class="input_end_css3" v-model="houseData.totalFloor" @keyup.native="changeTotalFloor" :disabled="isEdit" @change="changeValidate('floorInfo')" v-validate="'naturalNum|min(1)|max(99)'">
						<span slot="prefix">共</span>
						<span slot="suffix">层，</span>
					</a-input>
					<a-input placeholder="请输入" class="input_end_css4" v-model="houseData.perFloorRoomsTotal" @keyup.native="changeTotalFloor" :disabled="isEdit" @change="changeValidate('floorInfo')" v-validate="'naturalNum'">
        ...
        <div v-for="(item,index) in generatorRoomList" :key="index">
            ...
            <div class="room_item_info_set" v-show="item.floorNo == currentFloor">
                <div class="head_tabs_css">
                    <span class="tab_card_num" v-for="item in Number(generatorRoomList.length)"
                          :key="item" @click="currentFloor=item"
                          :class="currentFloor==item?'choosedTab':''">{{ item }}F</span>
                </div>
                <div class="floor_info_set" >
                    ...
                    <div class="set_btn_csss">
                        ...
                        <a-button
                            type="primary"
                            class="close_floor_btn"
                            size="small"
                            @click="disabledFloor"
                        >
                            <div v-show="item.isDisabled==0"><icon-font type="iconyumengtubiao_jinyong" />关闭楼层</div>
                            <div v-show="item.isDisabled==1"><icon-font type="iconyumengtubiao_chenggong"/>启用楼层</div>
                        </a-button>
                    </div>
                </div>
                <p class="room_num_css" v-show="item.isDisabled==0">房间号</p>
                <div class="room_num_item" v-show="item.isDisabled==0">
                    <span  v-for="(temp,tempIndex) in item.roomList" :key="tempIndex" :class="temp.isDisabled==0?'normalColor':'disabledColor'" @click="disabledHouse(temp)">{{ temp.roomNo }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="clearfix">
          <label-item title="所属门店(部门)" style="" :mustFill="true" class="u-bottom-border f-fl">
            <store-tree resMark="mk_house_centralized" @cancelOtherTree="cancelOtherTree(1)" :defaultValue='houseData.deptName' placement="topLeft" ref="selectDept" width="200px" @selectChange="selectDept" :show-employee="false"></store-tree>
          </label-item>
          <label-item title="负责人" class="f-fl" style="min-height: 100px" v-show="this.buildingProjectId!=''">
              <dept-employee-tree resMark="mk_house_centralized" @cancelOtherTree="cancelOtherTree(2)" ref="selectDeptEmp" width="200px" @selectChange="selectDeptEmp" :disableDept="true" placement="topLeft" :defaultValue="houseData.maintainerName"></dept-employee-tree>
          </label-item>
...
<script>
  ...
  import StoreTree from '@/components/DeptEmployeeTree/store';
    ...
    import roomMap from './addVillageByGaodeMap.vue';
	export default {
		name: "ContentStepOne",
		components: {
            headTitle,
