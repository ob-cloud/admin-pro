<template>
  ...
  <div class="hairRoom-box">
    ...
    <div class="describe" style="height: 38px;line-height: 20px;padding: 9px 0 9px 5px;">
      开启自动上架后，操作 新录房源、租客合同退租、手动变更房源状态时可租房源将自动上架到58租房平台上；关闭后，不会自动上架。
    </div>
    ...
    <div class="describe" style="height: 38px;line-height: 20px;padding: 9px 0 9px 5px;">
      开启定时上架后，房源每天按照设置的时间自动上架到58租房平台上；关闭将不会定时上架
    </div>
    <div class="checkBox clearfix">
      <div class="f-fl clearfix">
    ...
    <div class="describe" style="margin-right:0px;line-height: 20px;padding-top: 9px;">
      ① 设置统一呼叫中心后，来源于58租房平台的客户，联系管家时会直接联系到已设置好的呼叫中心人员；未被统一呼叫中心负责人覆盖的房源，上架时的管家还是默认为房源负责人
    </div>
    <div class="describe" style="margin: 0 0 0 14px;height: 29px;line-height: 20px;padding-bottom: 9px;">
      ② 不统一呼叫中心，默认上架到58租房平台的管家为房源负责人
    </div>
    <div class="checkBox clearfix">
      <div class="f-fl clearfix">
          ...
          <div class="house-data u-no-border" v-for="(item,index) in hairRoom3.value.customerList" :key="index">
            <div class="cityStyle" style="max-width: 207px;padding-left: 12px;">
              <city-town-tree ref="selectCityTown" :showTown="false" placeholder="请选择负责区域" placement="topLeft"
                              @selectChange="selectCityTownChange($event,index)" :defaultValue="item.areaInfo" :defaultCityId="item.cityId" :defaultAreaId="item.areaId" >
              </city-town-tree>
            </div>
            <div style="max-width: 222px;text-align: left;padding-left: 14px;">
              <span v-if="!item.cityId" style="color: #777777;">请先选择城市/区域</span>
              <a-select style="width: 180px;" v-model="item.maintainerId" placeholder="请选择负责人" v-if="item.cityId&&maintainerList[index].length>0" @change="getMaintainerInfo(index)">
                <a-select-option v-for="(item) in maintainerList[index]" :key="item.id">{{ item.name }}-{{item.phone}}</a-select-option>
              </a-select>
              <span v-if="item.cityId&&maintainerList[index].length==0">
                此城市发房账号未设置管家
              </span>
            </div>
    ...
    <div class="title clearfix">
      <div class="blueStyle f-fl"></div>
      <div class="titleFont f-fl">合租房源上架到58租房平台时，是否按照整租上架</div>
    </div>
    <div class="describe" style="margin-right:0px;line-height: 20px;padding-top: 9px;">
      ① 设置为"是"后：操作上架房源到58租房平台时，合租房源会以整租的形式和合租子间的形式同步上架到58租房平台上
    </div>
    <div class="describe" style="margin: 0 0 0 14px;height: 29px;line-height: 20px;padding-bottom: 9px;">
      ② 设置为"否"后：房源重新上架到58租房平台时，以整租形式上架到58租房平台上的房源会自动下架
    </div>
    <div class="checkBox clearfix">
      <div class="f-fl clearfix">
...
import {promotionListCityAgentCustomer} from "@/api/extension";
export default {
  name: "hairRoom",
  components: {
    CityTownTree,
  ...
  data() {
    return {
      ...
      hairRoomDefault1:{
        ...
        value:'0',
        module:'MARKETING',
        bizPoint:'PROMOTE_WUBA',
        mark:'HOUSE_WUBA_AUTO_PUSH',
        chooseType:2,
        id:'',
        comment:'',
        name:'58空置房源自动上架',
      ...
      hairRoomDefault2:{
        ...
        module:'MARKETING',
        bizPoint:'PROMOTE_WUBA',
        mark:'HOUSE_WUBA_TIMING_PUSH',
        chooseType:5,
        id:'',
        comment:'',
        name:'58空置、预到期、已到期房源定时上架',
      },
      hairRoomDefault3:{
        onOff:false,
        value:{
      ...
      maintainerList:[[]],
      cityAgentCustomerList:[],
    }
  },
  created() {
    this.getTimeList();
    this.promotionListCityAgentCustomer();
  },
  methods:{
    getMaintainerInfo(index){
      this.maintainerList[index].forEach((item)=>{
    ...
    deleteItem(index){
      ...
      this.maintainerList.splice(index,1);
    },
    AddItem(flag){
    ...
    save(data){
      //检验
      if(data.mark  == 'HOUSE_WUBA_TIMING_PUSH'&&data.onOff==1){
        if(this.hairRoom2.value.statusList.length == 0){
          this.$message.warning('请至少选择一种自动上架房源状态');
          return;
        }
      }
      if(data.mark  == 'WUBA_UNFIED_CUSTOMER'&&data.onOff==1){
        let temp = false;
        this.hairRoom3.value.customerList.forEach((item)=>{
          if(!item.cityId){
            temp = true;
    ...
    changeResrource(data,val){
      data.onOff = val;
      if(data.mark  == 'HOUSE_WUBA_TIMING_PUSH'){
        if(val == 0){
          data.value = {
            statusList:[1],
            hour:10,
      ...
      else if(data.mark  == 'WUBA_UNFIED_CUSTOMER'){
        if(val == 0){
          data.value = {
            customerList:[],
          };
        ...
        if (res.code === '200') {
          ...
          if(res.data && res.data.length > 0) {
            for (let i = 0; i < res.data.length; i++) {
              if(res.data[i].mark == 'HOUSE_WUBA_AUTO_PUSH'){
                this.hairRoom1 = res.data[i];
              }
              else if(res.data[i].mark == 'HOUSE_WUBA_TIMING_PUSH'){
                this.hairRoom2 = res.data[i];
                if(this.hairRoom2.value){
                  this.hairRoom2.value = JSON.parse(this.hairRoom2.value);
                }
              }
              else if(res.data[i].mark == 'WUBA_UNFIED_CUSTOMER'){
                this.hairRoom3 = res.data[i];
                if(this.hairRoom3.value){
                  this.hairRoom3.value = JSON.parse(this.hairRoom3.value);
                }
                if(this.hairRoom3.onOff==1){
                  //初始化集合
                  this.hairRoom3.value.customerList.forEach((item1,index)=>{
                    this.cityAgentCustomerList.forEach((item2)=>{
                      if(item1.cityId == item2.cityId){
                        this.maintainerList[index] = item2.agentCustomerList;
                      }
                    });
                  });
                }
              }
              else if(res.data[i].mark == 'WUBA_SHALE_AS_WHOLE'){
    ...
    selectCityTownChange(selectObj,index) {
      ...
      if(selectObj.cityId){
        this.cityAgentCustomerList.forEach((item)=>{
          if(selectObj.cityId == item.cityId){
            this.maintainerList[index] = item.agentCustomerList;
          }
        });
        // let parameter = {
        //   cityId:selectObj.cityId,
        //   bizPoint:'PROMOTE_WUBA',
        // };
        // promotionListAgentCustomer(parameter).then(res => {
        //   if (res.code === '200') {
        //     this.maintainerList[index] = res.data;
        //     this.maintainerList[index] = [
        //       {id:'1',name:'彭亚平',phone:'18070516305'},
        //       {id:'2',name:'小苹果',phone:'18070516304'},
        //       {id:'3',name:'高温',phone:'18170516304'},
        //     ];
        //   }
        // });
      }
    },
    promotionListCityAgentCustomer(){
      let parameter = {
        bizPoint:'PROMOTE_WUBA',
      };
      promotionListCityAgentCustomer(parameter).then(res => {
        if (res.code === '200') {
          this.cityAgentCustomerList = res.data;
          this.initDecisionList();
        }
      });
    },
