            ...
            <div>
                <div class="search-condition clearfix">
                    ...
                    <div class="f-fl clearfix" style="margin-top: 7px">
                        ...
                        <div class="f-fl">
                            <a-select v-model="sendData.periodMonth" placeholder="不限" @change="changePeriodMonth" style="width: 80px;margin-top: 2.5px">
...
<!--                    <p>实时监控主要租房群体是哪种性别</p>-->
            ...
            <div class="charts-list">
                ...
                <div class="charts-item" style="position: relative">
                    <p>付款方式分布</p>
                    <div v-if="!chart5Data||chart5Data.length==0" style="position:absolute;top:50%;left:calc(50% - 30px);color: #777777">暂无数据</div>
                    <div id="chart5"></div>
                </div>
            </div>
            <div class="charts-list">
        ...
        <div class="con" v-show="statusActiveIndex == 1">
            <a-table :loading="loading" size="middle" rowKey="id" :columns="columns" :dataSource="tableData.list" :pagination="false" bordered>
                ...
                <div slot="interestName" slot-scope="text">
                    {{text|blankVal}}
                </div>
    ...
    export default {
            ...
            return {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        dataIndex: 'ageName',
                        key: 'ageName',
                        scopedSlots: { customRender: 'ageName' },
                    },
                    {
                        title: '所属城市',
                        dataIndex: 'cityName',
                    ...
                    },
                    {
                        ...
                        key: 'interestName',
                        scopedSlots: { customRender: 'interestName' },
                    },
                    {
                        title: '付款方式',
                ...
                exportUrl: '/pc2b/da/renter_signer/list_export',
                exportData:{},
        ...
        methods: {
            ...
            initChart(val){
                ...
                this.initChart6Data(val);
            },
            initChart1Data(val){
                API.genderAnalysis(this.sendData).then(res => {
                    if (res.code === '200') {
            ...
            chart1(){
                let sum = 0;
                //percentage百分比相加之和一定要等于1
                this.chart1Data.forEach((item)=>{
                    item.percentage = item.percentage/100;
                    sum += item.quantity;
                })
                const ds = new DataSet();
                const dv = ds.createView().source(this.chart1Data);
                dv.transform({
                    type: 'map',
                    callback(row) {
                        row.value = parseInt(sum * row.percentage);
                        return row;
                    }
                });
                if(!this.chart1Model) {
                ...
                this.chart1Model.source(dv);
                this.chart1Model.coord('theta', {
                    radius: 0.75,
                    innerRadius: 0.6
                });
                let interval = this.chart1Model.intervalStack().position('percentage').color('name', [ '#59A8EB', '#E67F65' ])
                    .opacity(1)
                    .label('percentage', {
                        htmlTemplate: (val, item) => {
                            return (
                                `<div><p  style="white-space:nowrap;font-size: 10px;color: #777777">${item.point.name}</p><p><span style="white-space:nowrap;font-size: 10px;color: #111111">${(item.point.percentage*100).toFixed(2)}%</span><span style="white-space:nowrap;font-size: 10px;color: #777777">(${item.point.quantity}人)</span></p></div>`
                            )
                        }
                    }).style({
                        lineWidth: 1,
            ...
            initChart2Data(val){
                API.listAgeAnalysis(this.sendData).then(res => {
                    if (res.code === '200') {
            ...
            chart2(){
                ...
                const interval = this.chart2Model.intervalStack()
                    ...
                    .label('lable', {
                        htmlTemplate: (val, item) => {
                            return (
                                `<div><span style="white-space:nowrap;font-size: 10px;color: #111111">${(item.point.percentage*100).toFixed(2)}%</span><span style="white-space:nowrap;font-size: 10px;color: #777777">(${item.point.quantity}人)</span></div>`
                            )
                        }
                    })
                    .tooltip('name*quantity*percentage', (name, quantity, percentage) => {
            ...
            initChart3Data(val){
                API.listInterestAnalysis(this.sendData).then(res => {
                    if (res.code === '200') {
            ...
            initChart4Data(val){
                API.listProfessionAnalysis(this.sendData).then(res => {
                    if (res.code === '200') {
                        this.chart4Data = res.data;
                        this.chart4Data.sort(function (a, b) {
                            if (a.lable > b.lable) {
                                return -1;
            ...
            chart4(){
                ...
                this.chart4Model.coord().transpose();
                this.chart4Model.interval().position('name*quantity').size(10).color('name', [ '#59A8EB', '#18C2C2', '#F0BD68', '#82E6A0', '#5AD3ED', '#BBBBBB' ])
                    .opacity(1)
                    .label('quantity', {
                        textStyle: {
                            fill: '#8d8d8d'
            ...
            initChart5Data(val){
                API.listPeriodAnalysis(this.sendData).then(res => {
                    if (res.code === '200') {
            ...
            chart5(){
                ...
                this.chart5Model.coord('theta', {
                    radius: 0.65,
                    innerRadius: 0.6
                });
                const interval = this.chart5Model.intervalStack()
                    .position('percentage')
                ...
                interval.setSelected(this.chart5Data[0]);
            },
            updateChart5(){
                this.chart5Data.forEach((item)=>{
                    item.percentage = item.percentage/100;
            ...
            initChart6Data(val){
                API.listProvinceAnalysis(this.sendData).then(res => {
                    if (res.code === '200') {
            ...
            chart6(){
                this.chart6Model = new G2.Chart({
                    container: 'chart6',
                    forceFit: true,
                    height: 350,
                    padding: [ 30, 20, 30, 30 ]
                });
                this.chart6Model.source(this.chart6Data);
                this.chart6Model.axis('name', {
                    label: {
                        textStyle: {
                            fill: '#aaaaaa'
                        }
                ...
                this.chart6Model.legend(false);
                this.chart6Model.scale('quantity', {
                    alias: '人数'
                });
                this.chart6Model.interval().position('name*quantity').opacity(1).size(16)
                    .label('quantity', {
                        offset: 10,
                        textStyle: {
                            fill: '#595959',
                ...
                this.chart6Model.render();
            },
            updateChart6(){
                this.chart6Model.changeData(this.chart6Data);
            },
            renterSignerList() {
                if(this.sendData.periodMonth=='不限'){
                    this.sendData.periodMonth = undefined;
                }
                API.renterSignerList(this.sendData).then(res => {
                    if (res.code === "200") {
                        if(res.data == null){
                            this.tableData.list = [];
                        }else{
