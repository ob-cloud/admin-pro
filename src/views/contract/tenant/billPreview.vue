<template>
  <div class="bill-previe-box">
    <a-modal v-model="ownDialog.show" centered :footer="null" class="ownModalBorder titDialog" :width="ownDialog.width" :title="ownDialog.title" @cancel="closeThis">
      <div class="bill-preview-box">
          <div class="bill-preview">
            <div v-if="nameShow" style="background: rgba(245, 247, 254, 1);">
              <div class="nameShow-box f-clearfix">
                <p class="f-fl">系统根据租客电话自动检测到<span style="color:rgb(10, 135, 248)">{{reserveList.length}}</span>笔定金: 共计
                <span style="color:#FFA036">{{total}}</span>元;</p>
                <p class="f-fr" style="margin-right: 34.5px;">是否全部抵扣：<a-checkbox v-model="destineFul" @change="checkedAllYD" :disabled="list.length==0"></a-checkbox></p>
              </div>
              <div class="listContract-box">
                <div class="listContract f-clearfix" v-for="(item,i) in reserveList" :key="i">
                  <p class="f-fl listOne">定金：
                    <span style="color:#FFA036">{{item.money/100}}</span>元
                  </p>
                  <p class="f-fl listTwo">预定人：{{item.name}}-{{item.phone}}-{{item.certificateType==1?'身份证':item.certificateType==2?'护照':item.certificateType==3?'港澳通行证':item.certificateType==4?'台湾同胞证':item.certificateType==5?'军官证':item.certificateType==6?'驾驶证':'-'}}：{{item.certificateNo}} </p>
                  <P class="f-fr listTress" style="margin-right: 34.5px;"><a-checkbox v-model="item.checked" @change="checkItemYD" :disabled="list.length==0"></a-checkbox></P>
              </div>
              </div>
            </div>
              ...
              <div class="bill-item">
                <div class="bill-detail">
                  <div class="bill-title" style="font-size: 14px;">
                    ...
                    <div :style="showPaymentFlag?'max-width: 185px':'max-width: 206px'">操作</div>
                    <div style="max-width: 85px" v-if="showPaymentFlag">已收账单</div>
                  </div>
                    ...
                    <div class="bill-data u-no-border" v-for="(bill,index2) in bills.bills" :key="index2">
                      <div class="f-clearfix" style="display: flex;">
                        <div @click="showFinaceFu(index2,index)" class="bill-content-list cursor" :style="showPaymentFlag?'width: calc(100% - 84px);':'width:100%;'">
                          ...
                          <div style="color:#0A87F8" :style="showPaymentFlag?'max-width: 185px':'max-width: 206px'" class="cursor">查看明细</div>
                        </div>
...
import {
  listEnableDict, listByPhone, renterContract
...
export default {
  ...
  methods: {
    ...
    isUSeChange() {
      let list = [];
      this.reserveList.forEach((da, j) => {
        if (da.checked) {
          list.push(da.id)
        }
      });
      this.bookIdList = list;
      this.$emit('isUseBook', this.bookIdList);
      this.postData.bookIdList = this.bookIdList;
      renterContract(this.postData).then(res => {
        if (res.code === '200') {
          let data = [...res.data];
          this.init(data)
        } else {
    ...
    getData() {
      listByPhone(this.phone).then(res => {
        if (res.code === '200') {
          if (res.data.length == 0) {
            ...
            this.reserveList.forEach(function(e) {
              this.$set(e, 'checked', false);
              this.total = this.total + e.money / 100;
            }, this);
          }
        }
      })
    ...
    checkedAllYD() {
      // this.destineFul = !this.destineFul;
      this.reserveList.forEach((data, i) => {
        if (this.destineFul) {
            data.checked = true;
        } else {
            data.checked = false;
