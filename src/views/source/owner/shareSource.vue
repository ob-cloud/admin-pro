<template>
    <div class="shareSourceBox" :class="styleHeightLow?'height1':'height2'">
        <div class="share-source-detail">
            <div class="shared-company" v-if="sharedCompany">
                <span style="color: #777777;">已共享的合作公寓：</span>
                <span style="color: #111111">{{sharedCompany}}</span>
            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            initData(){
                API.ownerClueGetSharedPartner(this.sourceId).then(res=>{
                    if(res.data){
                        this.sharedCompany =res.data;
                    }
                });
            },
            cancelShareSource(){
            ...
            shareSoure(){
                ...
                API.ownerClueShare(this.sendData).then(res=>{
                    if (res.code == 200) {
