<template>
    <div @click.stop="showTree=true;isClick=true" class="tree-wrap">
        <div class="select-type-list" :class="{active: isClick}" :style="showAll?'border-bottom: 1px solid @borderColor;':'border-bottom: none;'">
            <ul class="f-clearfix" v-if="showAll">
                <template  v-if="selectObj.metaList && selectObj.metaList.length">
                    <li class="f-fl" @click.stop="deleteTags(item,index)" v-for="(item,index) in selectObj.metaList">
                        <span>{{item.name}}</span>
            ...
            <ul class="f-clearfix" v-else>
                <template  v-if="selectObj.metaList">
                     <div class="f-fl" v-for="(item,index) in selectObj.metaList">
        ...
        <div v-show="showTree" @click.stop="showTree=true;isClick=true" class="tree">
            <div class="content">
                <ul class="list">
                    <li @click="selectParent(item,index)" :class="{active: currentSelectParent.id==item.id}" v-for="(item,index) in costTypeListParent">
                        {{item.name}}
                        <icon-font v-show="currentSelectParent.id==item.id" type="iconyumengtubiao_xuanzhong-" />
                    </li>
                </ul>
                <ul class="list">
                    <li @click="selectChild(item,index)" :class="{active: isSelectAll ? index == 0 : item.isSelect}" v-for="(item,index) in costTypeListChild">
                        {{item.name}}
...
<script>
    ...
    export default {
        ...
        methods:{
            getCostTypeData(){
                PublicAPI.getGoodsType().then(res=>{
                    // console.log(res)
                    if (res.code == 200){
