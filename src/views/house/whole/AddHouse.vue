<template>
    <div id="addHouse" class="add-house">
        ...
        <div class="activeIndex clearfix">
            <div v-mark="['mk_house_index_tjfy']" class="f-fl normal cursor" :class="activeIndex==2?'click':''" @click="changeActiveIndex(2)">合租</div>
            <div v-mark="['mk_house_index_tjfy']" class="f-fl normal cursor" :class="activeIndex==1?'click':''" @click="changeActiveIndex(1)">整租</div>
            <div v-mark="['mk_house_centralized_tjxm']" class="f-fl normal cursor" :class="activeIndex==3?'click':''" @click="changeActiveIndex(3)">独栋</div>
        </div>
    </div>
</template>
...
<script>
    ...
    import AddCentralizedHouse from '../centralized/components/AddCentralizedHouse';
    export default {
        name: "addHouse",
        components: {
            AddWholeHouse,
        ...
        methods: {
          changeLoading(val){
            ...
            goConfig(val) {
                let flag = this.$refs.addCentralizedHouse.goConfig();
                if(flag&&val==3){
                    this.$emit('refreshPage');
                }
            },
