    ...
    <div class="assignUserBox clearfix">
        <div class="assign-user-detail" v-if="loadingFlag&&(!deptList||deptList.length==0)">
            <label-item title="选择指派的员工" :must-fill="true" style="min-height: 180px">
                <dept-employee-tree ref="selectDeptEmp" width="480px" :resMark="resMark" @cancelOtherTree="cancelCityTownTree" @selectChange="selectDeptEmp"></dept-employee-tree>
            </label-item>
        </div>
      <div class="assign-user-detail selectDeptStyle" v-if="loadingFlag&&deptList.length>0" style="margin-bottom: 100px;">
        ...
        <div class="buttonDiv f-fr">
            ...
            <a-button size="large" type="primary" class="formItemMarginButton saveFont" :loading="saveLoading" @click="assignUser">
                保存
            </a-button>
        </div>
    </div>
        ...
        methods: {
          ...
          getConfigData(){
            ...
            ownerClueAssigned(){
                ...
                API.ownerClueAssigned(data).then(res=>{
                    if(!res){
            ...
            delegateAssigned(){
                ...
                API.delegateAssigned(data).then(res=>{
                    if(!res){
                        this.saveLoading = false;
                    }
                    if (res.code === '200') {
            ...
            assignUser(){
                ...
                if(this.clueType==1||this.clueType==2){
                    ...
                    API.ownerClueAssignedLimitCheck(data).then(res=>{
                        if (res.code === '200') {
                            if(res.data.result==1){
                                this.ownerClueAssigned();
                            }
