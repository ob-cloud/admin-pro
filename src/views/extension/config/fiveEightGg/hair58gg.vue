<template>
  ...
  <div class="hairRoom-box">
    ...
    <div class="describe" style="height: 38px;line-height: 20px;padding: 9px 0 9px 5px;">
      开启自动上架后，操作 新录房源、租客合同退租、手动变更房源状态时可租房源将自动上架到58公寓馆租房平台上；关闭后，不会自动上架。
    </div>
    ...
    <div class="describe" style="height: 38px;line-height: 20px;padding: 9px 0 9px 5px;">
      开启定时上架后，房源每天按照设置的时间自动上架到58公寓馆租房平台上；关闭将不会定时上架
    </div>
    <div class="checkBox clearfix">
      <div class="f-fl clearfix">
    ...
    <div class="describe" style="margin-right:0px;line-height: 20px;padding-top: 9px;">
      ① 设置统一呼叫中心后，来源于58公寓馆平台的客户，联系管家时会直接联系到已设置好的呼叫中心人员；未被统一呼叫中心负责人覆盖的房源，上架时的管家还是默认为房源负责人
    </div>
    <div class="describe" style="margin: 0 0 0 14px;height: 29px;line-height: 20px;padding-bottom: 9px;">
      ② 不统一呼叫中心，默认上架到58公寓馆平台的管家为房源负责人
    </div>
    <div class="checkBox clearfix">
      <div class="f-fl clearfix">
...
export default {
  ...
  data() {
    return {
      ...
      hairRoomDefault1:{
        ...
        value:'0',
        module:'MARKETING',
        bizPoint:'PROMOTE_WUBAFLATS',
        mark:'HOUSE_WUBAFLATS_AUTO_PUSH',
        chooseType:2,
        id:'',
        comment:'',
        name:'58公寓馆空置房源自动上架',
      ...
      hairRoomDefault2:{
        ...
        module:'MARKETING',
        bizPoint:'PROMOTE_WUBAFLATS',
        mark:'HOUSE_WUBAFLATS_TIMING_PUSH',
        chooseType:5,
        id:'',
        comment:'',
        name:'58公寓馆空置、预到期、已到期房源定时上架',
      },
      hairRoomDefault3:{
        onOff:false,
        value:{
        ...
        module:'MARKETING',
        bizPoint:'PROMOTE_WUBAFLATS',
        mark:'WUBAFLATS_UNFIED_CUSTOMER',
        chooseType:5,
        id:'',
        comment:'',
        name:'58公寓馆统一客服',
  ...
  methods:{
    ...
    save(data){
      //检验
      if(data.mark  == 'HOUSE_WUBAFLATS_TIMING_PUSH'&&data.onOff==1){
        if(this.hairRoom2.value.statusList.length == 0){
          this.$message.warning('请至少选择一种自动上架房源状态');
          return;
        }
      }
      if(data.mark  == 'WUBAFLATS_UNFIED_CUSTOMER'&&data.onOff==1){
        let temp = false;
        this.hairRoom3.value.customerList.forEach((item)=>{
          if(!item.cityId){
            temp = true;
    ...
    changeResrource(data,val){
      data.onOff = val;
      if(data.mark  == 'HOUSE_WUBAFLATS_TIMING_PUSH'){
        if(val == 0){
          data.value = {
            statusList:[1],
            hour:10,
      ...
      else if(data.mark  == 'WUBAFLATS_UNFIED_CUSTOMER'){
        if(val == 0){
          data.value = {
            customerList:[],
          };
        ...
        if (res.code === '200') {
          ...
          if(res.data && res.data.length > 0) {
            for (let i = 0; i < res.data.length; i++) {
              if(res.data[i].mark == 'HOUSE_WUBAFLATS_AUTO_PUSH'){
                this.hairRoom1 = res.data[i];
              }
              else if(res.data[i].mark == 'HOUSE_WUBAFLATS_TIMING_PUSH'){
                this.hairRoom2 = res.data[i];
                if(this.hairRoom2.value){
                  this.hairRoom2.value = JSON.parse(this.hairRoom2.value);
                }
              }
              else if(res.data[i].mark == 'WUBAFLATS_UNFIED_CUSTOMER'){
