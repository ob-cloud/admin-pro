<template>
  <div class="agentList-box">
      ...
      <div class="basicsInfo">
        <div class="table">
          <div class="table-title">
            <div style="max-width: 153px;text-align: left;padding-left: 14px;">管家姓名</div>
            <div style="max-width: 177px;text-align: left;padding-left: 19px;">手机号</div>
            <div style="max-width: 170px;text-align: left;padding-left: 19px;">所属部门</div>
            <div style="max-width: 164px;text-align: left;padding-left: 19px;">角色</div>
            <div style="max-width: 93px;text-align: center;">操作</div>
          </div>
          <div class="house-data u-no-border" v-for="(item,index) in agentList" :key="index">
            <div style="max-width: 153px;text-align: left;padding-left: 14px;">{{ item.agentName }}</div>
            <div style="max-width: 177px;text-align: left;padding-left: 19px;">{{item.agentPhone}}</div>
            <div style="max-width: 170px;text-align: left;padding-left: 19px;">{{ item.deptName }}</div>
            <div style="max-width: 164px;text-align: left;padding-left: 19px;">{{ item.roleName?item.roleName:'-' }}</div>
            <div style="max-width: 93px;text-align: center;">
              <span class="cursor" v-mark="['mk_extension_config_jbzh']" @click="deleteAgent(item)" style="font-size: 14px;color: #FB4246;">解绑</span>
            </div>
          </div>
          <div class="house-data u-no-border cursor" @click="addAgent" v-mark="['mk_extension_config_bdzh']">
    ...
    <!-- 绑定管家 -->
    <a-modal destroyOnClose centered :footer="null" :maskClosable="false" width="520px" v-model="bindAgentDialog" class="ownModalBorder titDialog" title="绑定58租房推广平台管家" @cancel="bindAgentDialog=false;">
...
export default {
  ...
  created() {
    this.roleListAll();
    this.wubaListClientKeyAgent();
  },
  methods:{
    wubaDeleteAgent(id){
      API.wubaDeleteAgent({id:id}).then(res => {
        if (res.code === "200") {
    ...
    deleteAgent(item){
      let that = this;
      this.$modal.confirm({
        title: '确定要解除管家「'+ item.agentName +'-'+ item.agentPhone +'」与 58租房平台的绑定关系吗？',
        content:  '解绑后，该管家所负责的房源将全部从58租房平台上下架；且该管家所负责的房源将不可再推广到58租房平台上',
        width:"433px",
        height:"192px",
        okText: '确认',
        cancelText: '取消',
    ...
    wubaListClientKeyAgent(){
      API.wubaListClientKeyAgent(this.sendData).then(res => {
        if (res && res.code === "200") {
