<template>
  <div class="genBill-box">
    ...
    <div class="detail-info">
        ...
        <div class="table">
          ...
          <div class="house-data">
            <div class="u-no-border" style="max-width: 205px;text-align: left;padding-left: 20px;border-radius: 0px 0px 0px 4px;">
              <a-select v-model="waterFeeBill.bizType" style="width: 120px;">
      ...
      <div class="clearfix">
        <label-item class="u-bottom-border f-fl" title="付款人姓名" :mustFill="true" style="width: 167px;margin-right: 28px;">
          <a-input v-model="waterFeeBill.payerName" :maxlength="20" placeholder="请输入" />
        </label-item>
        <label-item class="u-bottom-border f-fl" title="付款人电话" :mustFill="true" style="width: 167px;margin-right: 28px;">
          <a-input v-model="waterFeeBill.payerPhone" :maxlength="11" @blur="checkPhone(waterFeeBill.payerPhone)" placeholder="请输入" />
        </label-item>
        <label-item class="u-bottom-border f-fl" title="付款账号">
          <div class="clearfix">
            <div class="f-fl" style="width: 74px">
              <a-select v-model="waterFeeBill.paymentAccountType" style="width: 74px">
            ...
            <div class="f-fl" v-if="waterFeeBill.paymentAccountType!=1" style="width: 291px">
              <a-input   v-model="waterFeeBill.paymentAccount" :maxlength="19" placeholder="请输入" />
            </div>
            <div class="f-fl clearfix" v-if="waterFeeBill.paymentAccountType == 1" style="width: 291px;position: relative;">
              <a-input class="f-fl" v-model="waterFeeBill.paymentAccount" @blur="addBankName" :maxlength="19" style="width:200px" placeholder="银行卡号" />
              <div class="dividerClass f-fl"></div>
              <a-input class="f-fl" v-model="waterFeeBill.bankName" :maxlength="19" style="width:91px;" placeholder="所属银行" />
            </div>
          </div>
        </label-item>
      </div>
      ...
      <div class="clearfix">
        <label-item class="f-fl" title="应收日期" :mustFill="true">
          <div class="u-bottom-border" style="width: 167px;margin-right: 30px;">
            <a-date-picker v-model="waterFeeBill.predictTime" :format="dateFormat" />
          </div>
        </label-item>
      ...
      <div style="margin:18px 0 22px 0;">
        <label-item class="u-bottom-border textAreaStyle" style="width: 760px;" title="账单备注" :maxSize="500" :valLength="waterFeeBill.comment.length">
          <a-textarea v-model="waterFeeBill.comment" maxlength="500" autosize placeholder="请输入备注信息" />
        </label-item>
      </div>
        ...
        <div class="clearfix">
          <label-item class="f-fl"  title="实收日期" :mustFill="true" style="width: 167px;margin-right: 28px;">
            <div class="u-bottom-border">
              <a-date-picker v-model="waterFeeBill.practicalTime" :format="dateFormat" />
            </div>
          </label-item>
          <label-item class="f-fl" title="支付途径" :mustFill="true" style="width: 167px;margin-right: 28px;">
            <div class="u-bottom-border">
              <a-select v-model="waterFeeBill.paymentMethod" style="width:100%" placeholder="请选择支付途径">
          ...
          <label-item class="f-fl" title="支付备注">
            <div class="u-bottom-border address">
              <a-input v-model="waterFeeBill.paymentNote" :autosize="{ minRows: 1, maxRows: 5 }" placeholder="请输入" /></a-input>
            </div>
          </label-item>
        </div>
      </div>
    ...
    <a-modal destroyOnClose centered :footer="null" :closable="false" :maskClosable="false" width="520px" v-model="loadingDialog" class="ownModalBorder" title="">
      <water-loading :resultData="resultData" resultDesc="正在充值，请稍后···" @setFailure="setFailure" @setSuccess="setSuccess"></water-loading>
    </a-modal>
  </div>
</template>
...
export default {
  ...
  created() {
    if(this.houseObj){
      ...
      this.waterFeeBill.houseId = this.houseObj.houseId;
      this.feeAmount = this.houseObj.feeAmount;
      this.resultType = 1;
      if(this.houseObj.renterName){
        this.waterFeeBill.payerName = this.houseObj.renterName;
      }
      if(this.houseObj.renterPhone){
        this.waterFeeBill.payerPhone = this.houseObj.renterPhone;
      }
      if(this.houseObj.renterPaymentType){
        this.waterFeeBill.paymentAccountType = this.houseObj.renterPaymentType;
      }
      if(this.houseObj.renterPaymentNo){
        this.waterFeeBill.paymentAccount = this.houseObj.renterPaymentNo;
      }
      if(this.houseObj.renterBank){
        this.waterFeeBill.bankName = this.houseObj.renterBank;
      }
      this.waterFeeBill.predictTime = this.$moment(new Date().getTime());
    }
    if(this.readingObj){
      this.houseAlias = this.readingObj.houseAlias;
      this.fullAddress = this.readingObj.fullAddress;
      this.waterFeeBill.houseId = this.readingObj.houseId;
      this.feeAmount = this.readingObj.feeAmount;
      if(this.readingObj.renterName){
        this.waterFeeBill.payerName = this.readingObj.renterName;
      }
      if(this.readingObj.renterPhone){
        this.waterFeeBill.payerPhone = this.readingObj.renterPhone;
      }
      if(this.readingObj.renterPaymentType){
        this.waterFeeBill.paymentAccountType = this.readingObj.renterPaymentType;
      }
      if(this.readingObj.renterPaymentNo){
        this.waterFeeBill.paymentAccount = this.readingObj.renterPaymentNo;
      }
      if(this.readingObj.renterBank){
        this.waterFeeBill.bankName = this.readingObj.renterBank;
      }
      if(this.readingObj.readTime){
        this.waterFeeBill.predictTime = this.$moment(this.readingObj.readTime);
      }
      if(this.readingObj.readTime&&this.readingObj.lastReadTime){
        this.waterFeeBill.beginTime = this.$moment(this.readingObj.lastReadTime);
        this.waterFeeBill.endTime = this.$moment(this.readingObj.readTime);
        let timeDateList = [this.waterFeeBill.beginTime,this.waterFeeBill.endTime];
  ...
  methods: {
    ...
    checkData(){
      if(!this.waterFeeBill.bizType){
        this.$message.warning('请选择费用承担方');
        return  false;
      }
      if(!this.waterFeeBill.feeSubjectId){
      ...
      if(!this.waterFeeBill.payerName){
        this.$message.warning('请输入付款人姓名');
        return  false;
      }
      if(!this.waterFeeBill.payerPhone){
        this.$message.warning('请输入付款人电话');
        return  false;
      }
      if(!this.waterFeeBill.predictTime){
        this.$message.warning('请选择应收日期');
        return  false;
      }
      if(!this.waterFeeBill.beginTime){
        this.$message.warning('请选择账单周期开始时间');
        return  false;
      }
      if(!this.waterFeeBill.endTime){
        this.$message.warning('请选择账单周期结束时间');
        return  false;
      }
      if(this.isPaid){
        //若是实收
        if(!this.waterFeeBill.practicalTime){
          this.$message.warning('请选择实收日期');
          return  false;
        }
        if(!this.waterFeeBill.paymentMethod){
    ...
    confirmGen(){
      ...
      if(checkDataResult){
        //金额装换成分
        this.waterFeeBill.feeAmount =  Math.round(this.feeAmount * 100) ;
        if(this.houseObj){
          let paramter = Object.assign({},this.chargeAmountData);
          if(this.isPaid){
            this.waterFeeBill.isPaid = 1;
          }
          else{
            this.waterFeeBill.isPaid = 0;
          }
          let annexIdList = [];
          ...
          this.waterFeeBill.annexIdList = annexIdList;
          if(this.$refs.voucherList){
            ...
            this.waterFeeBill.voucherIdList = voucherList;
          }
          paramter.waterFeeBill = this.waterFeeBill;
          //附件
          this.saveLoading = true;
          API.waterMeterChargeAmount(paramter).then(res => {
            if(!res){
        ...
        if(this.readingObj){
          ...
          let paramter = Object.assign({},this.waterFeeBill);
          if(this.isPaid){
            paramter.isPaid = 1;
          }
          else{
          ...
          //附件
          this.saveLoading = true;
          API.waterMeterReadRecordGenBill(paramter).then(res => {
            if(!res){
              this.saveLoading = false;
            }
            if(res.code === '200'){
    ...
    costSelect(value) {
      if (value[1]) {
        this.waterFeeBill.feeSubjectId = value[1];
      } else {
        this.waterFeeBill.feeSubjectId = value[0];
      }
    },
    getCost(){
          ...
          this.costTabsContent.forEach((item)=>{
            if(item.name == '能耗'){
              ...
              item.metaList.forEach((temp)=>{
                if(temp.name == '水费'&&this.readingObj.meterType==1){
                  this.elcostcascaderIds.push(temp.id);
                  this.waterFeeBill.feeSubjectId = temp.id;
                }
                if(temp.name == '热水费'&&this.readingObj.meterType==2){
                  this.elcostcascaderIds.push(temp.id);
                  this.waterFeeBill.feeSubjectId = temp.id;
                }
              });
    ...
    onTimeChange(date) {
      this.waterFeeBill.beginTime = date[0];
      this.waterFeeBill.endTime = date[1];
    },
    checkPhone(phone) {
      if(this.waterFeeBill.bizType ==1){
        if (!phone) {
    ...
    addBankName() {
      if (this.waterFeeBill.paymentAccount && this.waterFeeBill.paymentAccountType == 1) {
        getBankName(this.waterFeeBill.paymentAccount).then(res => {
          // if (res.code === "200") {
          if (res.data == null) {
            ...
            this.waterFeeBill.bankName = res.data;
