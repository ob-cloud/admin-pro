  ...
  <div class="agentDetail-box">
    <div class="agentDetail-title clearfix">
      <div class="agentDetail-title1 f-fl">
        <icon-font style="color: #FFFFFF;margin: 4px 0 0 4px;" type="icon58gongyuguanbeifen43x" />
      </div>
      <div class="agentDetail-title2 f-fl">
        "{{ accountInfo.cityName }}"58租房账号
      </div>
    ...
    <div class="agentDetail-content clearfix">
      <label-item class="item f-fl" title="clientId">
        <span class="itemFont">{{accountInfo.clientId?accountInfo.clientId:'-'}}</span>
      </label-item>
      <label-item class="item f-fl" title="clientSecret(密钥)">
...
export default {
  ...
  methods:{
    ...
    wubaDelete(){
      API.wubaDelete({id:this.clientKeyId}).then(res => {
        if (res.code === "200") {
    ...
    wubaView(){
      API.wubaView(this.clientKeyId).then(res => {
        if (res && res.code === "200") {
