    ...
    inserted: function (el, validateStr) {
        ...
        function checkedKeyupFun() {
            for (var i = 0; i < validateRuleArr.length; ++i) {
                let rateRegex = /^rate$/; // 判断设置了费率（1个小数点）
                let moneyZeroRegex = /^moneyZero$/; // 判断设置了money可以为0
                let moneyRegex = /^money$/; // 判断设置了money
                let moneyNoDotRegex = /^moneyNoDot$/; // 判断设置了money,不能有小数点
                let areaRegex = /^area$/; // 判断设置了面积area
                let maxlengthRegex = /maxlength\(/; //判断设置了 maxlength 最大长度
                let maxRegex = /max\(/; //判断设置了max 最大值
                let phoneRegex = /^phone$/; //判断设置了max 最大值
                let cardIDRegex = /^cardID$/; //判断设置了cardID 身份证
                let naturalNumRegex = /^naturalNum$/; //判断设置了naturalNum 自然数
                let positiveIntRegex = /^positiveInt$/; //判断设置了positiveInt 正整数
                let intRegex = /^int$/; //判断设置了int 整数
        ...
        function checkedBlurFun() {
            for (var i = 0; i < validateRuleArr.length; ++i) {
                let rateRegex = /^rate$/; // 判断设置了费率（1个小数点）
                let moneyZeroRegex = /^moneyZero$/; // 判断设置了金额money（可以为0）
                let moneyNoDotRegex = /^moneyNoDot$/; // 判断设置了金额money
                let moneyRegex = /^money$/; // 判断设置了金额money
                let areaRegex = /^area$/; // 判断设置了面积area
                let minRegex = /min\(/; //判断设置了min 最小值
                let minLengthRegex = /minlength\(/; //判断设置了 minlength 最小长度
                let phoneRegex = /^phone$/; //判断设置了max 最大值
                let cardIDRegex = /^cardID$/; //判断设置了cardID 身份证
                let intRegex = /^int$/; //判断设置了int 整数
        ...
        // blur时验证是否是正确的手机号码
        function cardIDBlur() {
            let cardIDRule = /(^[1-9]\d{5}(18|19|([23]\d))\d{2}((0[1-9])|(10|11|12))(([0-2][1-9])|10|20|30|31)\d{3}[0-9Xx]$)|(^[1-9]\d{5}\d{2}((0[1-9])|(10|11|12))(([0-2][1-9])|10|20|30|31)\d{2}[0-9Xx]$)/;
