<template>
    <div class="complete-clean clearfix">
        <!-- 完成 -->
        <div class="ipt-box" style="margin-top: 20px;" :style="showSubjectFlag?'padding: 0 20px':''">
            ...
            <div class="input-info" style="margin-top:5px;">
                <label-item class="u-bottom-border" title="保洁结果" :mustFill="true" style="width: 180px">
            ...
            <label-item class="u-bottom-border" :title="this.parData.completeIsSolve==0?'未完成原因':'保洁内容'" :mustFill="true" style="width: 100%;margin-top:20px;" :valLength="parData.cleanContent.length" :maxSize="200">
                <a-textarea style="min-height: 30px;" v-model="parData.cleanContent" maxlength="200" :autosize="{ minRows: 1, maxRows: 5 }" placeholder="请输入" />
            </label-item>
            <div style="margin: 20px 0 10px 0;"><span style="font-size: 12px;color: #777777;">附件</span><span style="font-size: 12px;color: #FFA036;">（附件支持格式：jpg、png、jpeg、mp4、avi、mov）</span></div>
        ...
        <div class="ipt-box" style="margin-top: 20px;" v-if="showSubjectFlag">
            ...
            <div class="basicsInfo">
                <div class="table margin-top-13">
                    ...
                    <div class="house-data u-no-border cursor" @click="AddSubjectObj(true)" v-if="cleanSubject.subjectList.length==0">
                        <div class="continueAddStyle">
                            <icon-font type="iconyumengtubiao_tianjia" />添加
                        </div>
                    </div>
                    <div class="house-data u-no-border cursor" @click="AddSubjectObj(false)" v-if="cleanSubject.subjectList.length>0">
                        <div class="continueAddStyle">
...
export default {
    ...
    methods: {
        ...
        AddSubjectObj(firstFlag){
            ...
            if(!firstFlag){
                let subjectList = this.cleanSubject.subjectList;
                let temp = false;
        ...
        moduleFieldConfigGet(val){
            ...
            moduleFieldConfigGet(paramter).then(res=>{
                if(res&&res.data&&res.data.fieldsObj){
                    this.cleanSubject = res.data.fieldsObj;
                }
            });
        },
        cancelPreviewImage() {
        ...
        save() {
            ...
            if (!this.parData.cleanContent) {
                this.$message.warning('请填写保洁内容！');
                return;
            }
            //图片处理
            ...
            API.cleanComplete(data).then(res => {
                if (res.code === "200") {
