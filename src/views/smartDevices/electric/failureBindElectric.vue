<template>
  <div class="bindElectric-box">
    <div class="bindElectric-box-top clearfix" >
      <div class="title f-fl">请选择电表的品牌/类型</div>
    ...
    <div class="bindElectric-box-accounts">
      <div class="bindLick-account clearfix">
        <div class="itemStyle cursor f-fl" :class="bindForm.brand==0?'clickStyle':'unClickStyle'" @click="changeOtherBrand()">
          ...
          <div class="title f-fl" style="height: 34px;line-height: 34px;margin-left: 11px;">普通电表</div>
        ...
        <div v-if="authAccountList.length>0" class="itemStyle cursor f-fl" v-for="(item) in authAccountList" :key="item.id" @click="changeBrand(item)" :class="bindForm.authAccountId==item.id?'clickStyle':'unClickStyle'">
          ...
          <div class="chaoyiStyle svgStyle f-fl" v-if="item.brand == 4"><icon-font  type="iconchaoyi"></icon-font></div>
          <div class="f-fl" style="margin-left: 10px;">
            ...
            <div class="title overHidde" style="line-height: 18px;max-width: 120px;" v-else>{{item.thirdPartyAccountId}}</div>
        ...
        <div class="itemStyle cursor f-fl" style="background: #F6F7F8;" v-if="!acceptPlat.fengDian" @click="goToAuth(3)">
    ...
    <div class="clearfix" style="width: 1260px;" v-if="bindForm.brand==0">
      ...
      <div class="set_info f-clearfix">
        ...
        <div class="clearStyle f-fl cursor" @click="clearSearchListUnboundElectricHouse">
          清空
        </div>
      </div>
      <div class="basicsInfo" :style="tableData.length==0?'padding-bottom: 20px;':''">
        <div class="table">
          <div class="table-title">
            ...
            <div style="max-width: 113px;text-align: left;padding-left: 9px;">面积</div>
            ...
            <div class="cursor" style="max-width: 225px;text-align: left;padding-left: 14px;" @click="goBatchSetOne()">
              表数统计方式/电费单价<icon-font style="color: #0A87F8;margin-left: 5px;" type="iconyumengtubiao_shezhi"/>
            </div>
            <div style="max-width: 300px;text-align: left;padding-left: 14px;border-radius: 0px 4px 0px 0px;">电表描述</div>
          </div>
          <div class="house-data u-no-border" v-for="(item,index) in tableData" :key="item.houseId"
            ...
            <div class="priceStyle clearfix" :class="item.name&&item.clickSave&&!item.unitPrice?'mustStyle':''"
              ...
              <div class="f-fl" style="width: 80px;">
                <a-input v-mustFill:[item.clickSave] :class="item.clickSave&&item.name&&!item.unitPrice?'borderRed':''" v-validate="'money|max(9.99)'" v-model="item.unitPrice" placeholder="单价" @change="getMustFill(index)"></a-input>
              </div>
              <div class="f-fr">
    ...
    <div class="smartDeviceStyle clearfix" style="border-bottom: 8px solid #F6F7F8;" v-else>
          ...
          <div class="table">
            <div class="table-title">
              ...
              <div class="cursor" @click="goBatchSetTwo()" style="max-width: 100px;text-align: left;padding-left: 14px;border-radius: 0px 4px 0px 0px;">
                电费单价<icon-font style="color: #0A87F8;margin-left: 5px;" type="iconyumengtubiao_shezhi"/>
              </div>
            </div>
            <div class="house-data u-no-border" v-for="(item,index) in tableData" :key="item.houseId"
                 @mouseover="mouseOverHouseId=item.houseId;" @mouseleave="mouseOverHouseId=''"
              ...
              <div class="clearfix cursor" style="max-width: 228px;text-align: left;padding: 0 10px;" :style="mouseOverHouseId==item.houseId?'background:#E6F3FE':''" @click="setSelect(item)">
                ...
                <div class="f-fl" style="width: 120px;margin-left: 10px;">
                    <span v-if="!item.sn" >
                      <span v-if="houseIdList.indexOf(item.houseId)>-1" style="color: #FB4246">请选择右边的电表</span>
                      <span v-else style="color: #BBBBBB">请选择右边的电表</span>
                    </span>
              ...
              <div class="priceStyle clearfix" style="max-width: 100px;text-align: left;padding: 0 10px;">
                <div class="f-fl" style="width: 60px;">
                  <a-input v-mustFill:[item.clickSave] :class="item.clickSave&&!item.unitPrice?'borderRed':''" v-validate="'money|max(9.99)'" v-model="item.unitPrice" placeholder="单价" @change="getMustFill_smart(index)"></a-input>
                </div>
                <div class="f-fr">
                  元
                </div>
      ...
      <div class="f-fl yunDingElectricList-box" v-if="bindForm.brand==1">
        <div style="padding: 19.5px 0 14.5px 0;">
          <div class="title">电表信息</div>
        </div>
        <div class="detail_info">
          <div class="set_info f-clearfix">
            <div class="selectItem f-fl cityTree">
              <div class="titleTip1 titleTip1With1 f-fl">城市/区域</div>
              <city-town-tree class="f-fl" style="width: 64px;" placeholder="不限" ref="selectCityTown" :showArea="true" :showTown="false" @selectChange="selectCityTownChange_info"></city-town-tree>
            </div>
            <div class="selectItem f-fl">
              <div class="titleTip1 titleTip1With2 f-fl">房源名称</div>
              <a-input placeholder="请输入" v-model="sendData_info.houseName" style="width: 64px;height: 34px" @pressEnter="eletricList"></a-input>
            </div>
            <div class="selectItem f-fl">
              <div class="titleTip1 titleTip1With3 f-fl">小区名</div>
              <a-input placeholder="请输入" v-model="sendData_info.communityName" style="width: 68px;height: 34px" @pressEnter="eletricList"></a-input>
            </div>
            <div class="selectItem f-fl">
              <div class="titleTip1 titleTip1With2 f-fl">详细地址</div>
              <a-input class="overHidde" placeholder="请输入" v-model="sendData_info.houseAddress" @pressEnter="eletricList" style="width: 64px;height: 34px"></a-input>
            </div>
            <div class="selectItem searchButtonStyle cursor f-fl" @click="eletricList" style="margin-right: 0px;">
              <icon-font type="iconyumengtubiao_sousuo" />
            </div>
          </div>
          <div class="electricList_info">
            <div class="basicsInfo" :style="tableData_info.length==0?'padding-bottom: 20px;':''">
              <div class="table">
                <div class="table-title">
                  <div style="max-width: 299px;text-align: left;padding-left: 9px;border-radius: 4px 0px 0px 0px;">安装地址</div>
                  <div style="max-width: 200px;text-align: left;padding-left: 9px;">电表序列号</div>
                ...
                <div class="house-data u-no-border" v-for="(item) in tableData_info" :key="item.houseId">
                  <div class="item" style="max-width:196px;text-align: left;padding-left: 9px;display: flex;align-items: center;" :style="item.roomList.length==1||item.roomList.length==0?'height:60px;line-height:60px;':'height:'+item.roomList.length*40+'px;'">
                  ...
                  <div class="item" v-if="item.roomList.length==0" style="text-align: center;max-width: 658px;height:60px;line-height: 60px;color: #BBBBBB">
                    暂无数据
                  </div>
                  <div class="item" v-else style="max-width: 366px;" :style="item.roomList.length==1?'height:60px;line-height:60px;':''">
                    <div class="itemDetail" v-for="(temp,index) in item.roomList">
                      <div style="max-width: 103px;text-align: left;padding-left: 9px;">
                        {{temp.roomName}}
                      </div>
                      <div style="max-width: 200px;text-align: left;padding-left: 9px;">
                        {{temp.sn?temp.sn:'-'}}
                      </div>
                      <div style="max-width: 56px;text-align: center;background: #F7F8FB;font-size: 14px;color: #BBBBBB;" v-if="!temp.isHaveElectricMeter||temp.isHaveElectricMeter==0">
                        无电表
                      </div>
                      <div style="max-width: 56px;text-align: center;background: #F7F8FB;font-size: 14px;color: #BBBBBB;" v-if="temp.isHaveElectricMeter&&temp.isHaveElectricMeter==1&&temp.isBound==1">
                        已绑定
                      </div>
                      <div class="checkStyle cursor" v-if="temp.isHaveElectricMeter&&temp.isHaveElectricMeter==1&&temp.isBound==0" @click="selectedElectricUuidFn(item,temp)" style="max-width: 56px;text-align: center;">
                        <icon-font class="selectStyle" v-if="selectedElectricIds.indexOf(temp.uuid)>-1" type="iconyumengtubiao_danxuan-yixuanze" />
      ...
      <div class="f-fl yunDingElectricList-box" v-if="bindForm.brand==3">
        <div style="padding: 19.5px 0 14.5px 0;">
          <div class="title">
            电表信息
            <span style="color: #FFA036;font-size: 14px;margin-left: 20px;">如找不到您要的设备，请点击“同步设备数据”</span>
            <span class="cursor" @click="freshData_fengDian" style="color: #0A87F8;font-size: 14px;margin-left: 30px;"><icon-font :class="{'imgrotate_common':syncInfoLoading}" style="margin-right: 3px;" type="iconyumengtubiao_zhongzhi" />同步设备数据</span>
          </div>
        </div>
        <div class="detail_info">
          <div class="set_info f-clearfix" style="margin-bottom: 20px;">
            <div class="selectItem f-fl">
              <div class="titleTip1 titleTip1With2 f-fl" style="width: 70px;">设备序列号</div>
              <a-input placeholder="请输入" v-model="sendData_info.sn" style="width: 126px;height: 34px" @pressEnter="eletricList"></a-input>
            </div>
            <div class="selectItem f-fl">
              <div class="titleTip1 titleTip1With2 f-fl">详细地址</div>
              <a-input class="overHidde" placeholder="请输入" v-model="sendData_info.fuzzyKeyword" style="width: 254px;height: 34px" @pressEnter="eletricList"></a-input>
            </div>
            <div class="selectItem searchButtonStyle cursor f-fl" @click="eletricList" style="margin-right: 0px;">
              <icon-font type="iconyumengtubiao_sousuo" />
            </div>
          ...
          <div class="electricList_info">
            <div class="basicsInfo" :style="tableData_info.length==0?'padding-bottom: 20px;':''">
              <div class="table">
                <div class="table-title">
                  <div style="max-width: 379px;text-align: left;padding-left: 9px;border-radius: 4px 0px 0px 0px;">安装地址</div>
                  <div style="max-width: 120px;text-align: left;padding-left: 9px;">电表序列号</div>
                  <div style="max-width: 56px;text-align: center;border-radius: 0px 4px 0px 0px;">选择</div>
                </div>
                <div class="house-data u-no-border" v-for="(item) in tableData_info" :key="item.houseId">
                  <div class="item" style="text-align: left;padding-left: 9px;display: flex;align-items: center;" :style="item.meterInfoList.length==0?'height:60px;line-height:60px;max-width:304px;':item.meterInfoList.length==1?'height:60px;line-height:60px;max-width:196px;':'height:'+item.meterInfoList.length*40+'px;max-width:196px;'">
                    <div :style="item.meterInfoList.length==0?'height:60px;line-height:60px;':'line-height:20px;'">
                      <span style="font-size: 13px;color: #777777;">{{item.houseAddress}}</span>
                    </div>
                  </div>
                  <div class="item" v-if="item.meterInfoList.length==0" style="text-align: center;max-width: 256px;height:60px;line-height: 60px;color: #BBBBBB;border-right: 1px solid #EEEEEE;">
                    暂无电表
                  </div>
                  <div class="item" v-else style="max-width: 366px;" :style="item.meterInfoList.length==1?'height:60px;line-height:60px;':''">
                    <div class="itemDetail" v-for="(temp,index) in item.meterInfoList">
                      <div style="max-width: 183px;text-align: left;padding-left: 9px;">
                        {{temp.name}}
                      </div>
                      <div style="max-width: 120px;text-align: left;padding-left: 9px;">
                        {{temp.sn?temp.sn:'-'}}
                      </div>
                      <div style="max-width: 56px;text-align: center;background: #F7F8FB;font-size: 14px;color: #BBBBBB;" v-if="!temp.isHaveElectricMeter||temp.isHaveElectricMeter==0">
                        无电表
      ...
      <div class="f-fl yunDingElectricList-box" v-if="bindForm.brand==4">
        <div style="padding: 19.5px 0 14.5px 0;">
          <div class="title">
            电表信息
            <span style="color: #FFA036;font-size: 14px;margin-left: 20px;">如找不到您要的设备，请点击“同步设备数据”</span>
            <span class="cursor" @click="freshData_chaoyi" style="color: #0A87F8;font-size: 14px;margin-left: 30px;"><icon-font :class="{'imgrotate_common':syncInfoLoading}" style="margin-right: 3px;" type="iconyumengtubiao_zhongzhi" />同步设备数据</span>
          </div>
        </div>
        <div class="detail_info">
          <div class="set_info f-clearfix">
            ...
            <div class="selectItem f-fl">
              ...
              <a-input placeholder="请输入" v-model="sendData_info.deviceNo" style="width: 70px;height: 34px" @pressEnter="eletricList"></a-input>
            </div>
            <div class="selectItem f-fl">
              <div class="titleTip1 titleTip1With2 f-fl">详细地址</div>
              <a-input class="overHidde" placeholder="请输入" v-model="sendData_info.fuzzyKeyword" style="width: 134px;height: 34px" @pressEnter="eletricList"></a-input>
            </div>
            <div class="searchButtonStyle cursor f-fl" @click="eletricList" style="margin-right: 0px;">
              <icon-font type="iconyumengtubiao_sousuo" />
            </div>
          </div>
          <div class="electricList_info">
              ...
              <div class="table">
                ...
                <div class="house-data u-no-border" v-for="(item) in tableData_info" :key="item.houseId">
                  ...
                  <div class="checkStyle cursor" @click="selectedElectricUuidFn_chaoyi(item)" style="max-width: 56px;text-align: center;border-right: 1px solid #EEEEEE;">
                    <icon-font class="selectStyle" v-if="selectedElectricIds.indexOf(item.deviceNo)>-1" type="iconyumengtubiao_danxuan-yixuanze" />
                    <icon-font class="unSelectStyle" v-else type="iconyumengtubiao_danxuan-weixuanze" />
                  </div>
                </div>
              </div>
            ...
            <div class="contFoot" >
              <a-pagination v-show="tableData.length>0" size="small" v-model="pagination_info.current" :total="pagination_info.total" :showTotal="total => `共 ${total} 条`" :pageSize="pagination_info.pageSize" showSizeChanger @change="onSizeChange_chaoyi" @showSizeChange="onShowSizeChange_chaoyi" />
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="clearfix" style="margin-top: 15px;">
      <div class="buttonDiv f-fr">
        ...
        <a-button size="large" type="primary" class="formItemMarginButton saveFont" :loading="saveLoading" @click="bindElectric">
          确定绑定
        </a-button>
      </div>
    </div>
...
export default {
  ...
  data(){
    return {
      ...
      bindForm:{
        ...
        mixBrandElectricMeterList:[], //杂牌电表绑定数据列表
        yundingElectricMeterList:[],  //云丁
        powerbeeElectricMeterList:[], //蜂电
      },
      saveLoading:false,
      mixBrandElectricMeterList:[],
      selectedElectricList:[],    //选中的一对项
  ...
  created() {
    //初始化原数据
    this.bindForm.brand = this.bindElectricBrand;
    this.bindForm.authAccountId = this.bindElectricAuthAccountId;
  ...
  methods: {
    onShowSizeChange_chaoyi(current, pageSize){
      this.sendData_info.current = current;
      this.sendData_info.size = pageSize;
      this.pagination_info.pageSize = pageSize;
      this.pagination_info.current = current;
      this.eletricList();
    },
    onSizeChange_chaoyi(current){
      this.sendData_info.current = current;
      this.pagination_info.current = current;
      this.eletricList();
    },
    ...
    selectedElectricUuidFn_chaoyi(item){
      let pos = this.selectedElectricIds.indexOf(item.uuid);
      if(pos>-1){
        this.selectedElectricIds.splice(pos,1);
        let selectedElectricList = [];
        this.selectedElectricList.forEach((electric)=>{
          if(electric.uuid == item.uuid){
            ...
            selectedElectricList.push(electric);
          }
        });
        this.selectedElectricList = selectedElectricList;
      }
      else{
        //检查数据是否完整
        let complete = false;
        ...
        if(!complete){
          this.$message.warning('请先选择需要绑定电表的房源');
          return ;
        }
        this.selectedElectricList.forEach((electric)=>{
          if(!electric.uuid){
            if(this.bindForm.brand==4){
              electric.fullAddress = item.fullAddress;
              electric.deviceType = item.deviceType;
              electric.productNo = item.productNo;
              electric.uuid = item.uuid;
              electric.deviceId = item.deviceId;
              electric.sn= item.uuid;
            }
            this.selectedElectricIds.push(item.uuid);
            this.tableData.forEach((house)=>{
              if(house.houseId == electric.houseId){
                house.sn = item.uuid;
    ...
    selectedElectricUuidFn(item,temp){
      let pos = this.selectedElectricIds.indexOf(temp.uuid);
      if(pos>-1){
        this.selectedElectricIds.splice(pos,1);
        let selectedElectricList = [];
        this.selectedElectricList.forEach((electric)=>{
          if(electric.uuid == temp.uuid){
            this.tableData.forEach((house)=>{
              if(house.houseId == electric.houseId){
                house.sn = '';
                house.uuid = '';
        ...
        this.selectedElectricList.forEach((electric)=>{
          if(!electric.uuid){
            if(this.bindForm.brand==1){
              ...
              electric.uuid = temp.uuid;
              electric.sn = temp.sn;
            }
            else if(this.bindForm.brand==3){
              electric.houseAddress = item.houseAddress;
              electric.deviceHousePid = item.houseId;
              electric.deviceHouseId = temp.roomId;
              electric.uuid = temp.uuid;
              electric.address = temp.address;
              electric.gwUuid = temp.gwUuid;
              electric.isBound = temp.isBound;
              electric.address = temp.address;
              electric.name = temp.name;
              electric.onlineStatus = temp.onlineStatus;
              electric.paymentMode = temp.paymentMode;
              electric.powerUsed = temp.powerUsed;
              electric.powerUsedRefTime = temp.powerUsedRefTime;
              electric.sn = temp.sn;
              electric.status = temp.status;
              electric.deviceId = temp.deviceId;
            }
            this.selectedElectricIds.push(temp.uuid);
            this.tableData.forEach((house)=>{
              if(house.houseId == electric.houseId){
                house.sn = temp.sn;
    ...
    listPullYundingElectricMeter(){
      this.rightDataLoading = true;
      API.listPullYundingElectricMeter(this.sendData_info).then(res => {
    ...
    listPullPowerbeeElectricMeter(){
      this.rightDataLoading = true;
      API.listPullPowerbeeElectricMeter(this.sendData_info).then(res => {
        this.rightDataLoading = false;
        if (res&&res.code === "200") {
          this.tableData_info = res.data.records;
          this.pagination_info.total = res.data.total;
    ...
    listPullJoyElectricMeter(){
      this.rightDataLoading = true;
      API.listPullJoyElectricMeter(this.sendData_info).then(res => {
        this.rightDataLoading = false;
        if (res&&res.code === "200") {
          this.tableData_info = res.data.records;
          this.tableData_info.forEach((item)=>{
            item.uuid = item.deviceNo;
          });
          this.pagination_info.total = res.data.total;
          this.sendData_info.lastDataRowIndex = res.data.lastDataRowIndex;
        }
    ...
    setSelect(item){
      ...
      if(this.houseIdList.indexOf(item.houseId)>-1){
        ...
        let uuid = '';
        let selectedElectricList = [];
        this.selectedElectricList.forEach((temp)=>{
          if(temp.houseId == item.houseId){
            ...
            selectedElectricList.push(temp);
          }
        });
        this.selectedElectricList = selectedElectricList;
        if(uuid){
          let tempPos = this.selectedElectricIds.indexOf(uuid);
          if(tempPos>-1){
            this.selectedElectricIds.splice(tempPos,1);
          }
        }
      }
      else{
        ...
        if(flag){
          this.$message.warning('请选择「' + fullAddress + '」绑定的电表');
          return ;
        }
        ...
        this.selectedElectricList.push(obj);
      }
      this.$forceUpdate();
    },
    eletricList(){
      if(this.bindForm.brand==1){
        this.listPullYundingElectricMeter();
      }
      else if(this.bindForm.brand==3){
        this.listPullPowerbeeElectricMeter();
      }
      else if(this.bindForm.brand==4){
        this.listPullJoyElectricMeter();
      }
    },
    setSuccessTwo(obj){
      ...
      this.selectedElectricList.forEach((item)=>{
        item.unitPrice = this.batchSetTwoObj.unitPrice;
      });
    },
    setSuccessOne(obj){
      ...
      let houseIdList = [];
      this.mixBrandElectricMeterList.forEach((item)=>{
      ...
      this.tableData.forEach((item)=>{
        ...
        if(houseIdList.indexOf(item.houseId)==-1){
          this.mixBrandElectricMeterList.push(obj);
        }
        else{
          this.mixBrandElectricMeterList.forEach((item)=>{
            item.billingMethod = this.batchSetOneObj.billingMethod;
    ...
    bindElectric(){
      if(this.bindForm.brand == 0){
        ...
        if(this.mixBrandElectricMeterList.length==0){
          this.$message.warning('请至少填写一条数据');
          return ;
        }
        //构造数据
        let mixBrandElectricMeterList = [];
        this.mixBrandElectricMeterList.forEach((item)=>{
          let obj ={
          ...
          mixBrandElectricMeterList.push(obj);
        });
        let parameter ={
          authAccountId:'',
          brand:0,
          yundingElectricMeterList:[],
          guojiaLockList:[],
          mixBrandElectricMeterList:mixBrandElectricMeterList,
        };
        this.saveLoading = true;
        API.electricMeterBatchBind(parameter).then(res=>{
          if(!res){
      ...
      if(this.bindForm.brand == 1){
        if(this.selectedElectricList.length==0){
          this.$message.warning('请至少填写一条数据');
          return ;
        }
        let selectedElectricList = [];
        ...
        this.selectedElectricList.forEach((item)=>{
          ...
          if(item.uuid&&item.unitPrice){
            ...
            selectedElectricList.push(obj);
          }
        });
        if(compeleteFlag){
          this.$message.warning('请完善电费单价');
          return ;
        }
        if(noUuid){
          this.$message.warning('存在未绑表房源');
          return ;
        }
        let parameter ={
          authAccountId:this.bindForm.authAccountId,
          brand:this.bindForm.brand,
          yundingElectricMeterList:selectedElectricList,
          mixBrandElectricMeterList:[],
        };
        this.saveLoading = true;
        API.electricMeterBatchBind(parameter).then(res=>{
      ...
      if(this.bindForm.brand == 3){
        ...
        this.selectedElectricList.forEach((item)=>{
          if(item.uuid&&item.unitPrice){
            ...
            obj.uuid= item.uuid;
            obj.unitPrice= item.unitPrice;
            obj.gwUuid = item.gwUuid;
            obj.name = item.name;
            obj.onlineStatus = item.onlineStatus;
            obj.paymentMode = item.paymentMode;
            obj.powerUsed = item.powerUsed;
            obj.powerUsedRefTime = item.powerUsedRefTime;
            obj.sn = item.sn;
        ...
        let parameter ={
          ...
          mixBrandElectricMeterList:[],
          guojiaLockList:[],
          powerbeeElectricMeterList:selectedElectricList,
        };
        this.saveLoading = true;
        API.electricMeterBatchBind(parameter).then(res=>{
          if(!res){
          ...
          if(res.code === '200'){
            this.saveLoading = false;
            this.$emit('bindComplete',res.data,3,this.bindForm.authAccountId)
          }
          else{
            this.$message.error('绑表失败');
            this.saveLoading = false;
      ...
      if(this.bindForm.brand == 4){
        ...
        let parameter ={
          ...
          joyElectricMeterList:selectedElectricList,
        };
        this.saveLoading = true;
        API.electricMeterBatchBind(parameter).then(res=>{
          if(!res){
    ...
    clearSearchListUnboundElectricHouse(){
    ...
    getMustFill_smart(index){
      ...
      this.tableData.forEach((item)=>{
        ...
        this.selectedElectricList.forEach((electric)=>{
          if(electric.houseId==item.houseId){
            electric.unitPrice = item.unitPrice;
          }
        });
      });
    },
    ...
    changeBrand(item){
      ...
      this.selectedElectricIds = [];
      //清空杂牌的数据
      this.mixBrandElectricMeterList = [];
      //重置搜索条件
      this.initSearchConHouse();
      this.listUnbindSmartDevice();
      this.initSearchCon();
      ...
      this.tableData.forEach((item)=>{
        if(item.unitPrice){
          ...
          if(pos ==-1){
            ...
            this.mixBrandElectricMeterList[pos] = obj;
          }
        }
      });
    },
    ...
    listUnbindSmartDevice(){
      this.sendData.houseIdList = this.bindElectricResult.failHouseIdList;
        ...
        if (res&&res.code === "200") {
          ...
          this.tableData.forEach(function(element, index) {
            ...
            if(this.bindForm.brand==0){
              ...
              if(pos>-1){
                this.selectedElectricList.forEach((selectedElectric)=>{
                  if(selectedElectric.houseId == element.houseId){
                    element.unitPrice = selectedElectric.unitPrice;
                    element.sn = selectedElectric.sn;
                    element.uuid = selectedElectric.uuid;
                  }
                });
              }
            }
            else if(this.bindForm.brand==3||this.bindForm.brand==4){
              if(this.batchSetTwoObj&&this.batchSetTwoObj.unitPrice){
                element.unitPrice = this.batchSetTwoObj.unitPrice;
              }
              let pos = this.houseIdList.indexOf(element.houseId);
              if(pos>-1){
                this.selectedElectricList.forEach((selectedElectric)=>{
                  if(selectedElectric.houseId == element.houseId){
                    ...
                    element.sn = selectedElectric.sn;
                    element.uuid = selectedElectric.uuid;
                    element.deviceId = selectedElectric.deviceId;
                  }
                });
              }
            }
...
.bindElectric-box{
  ...
  .bindElectric-box-top{
    margin-top: 16.5px;
  }
  .bindElectric-box-accounts{
