<template>
    <div class="transfer-box">
        <div class="transfer-line1" v-if="!successFlag">
            <span class="transfer-line1-font">对方钱包信息：</span><span class="transfer-line2-font">{{transferPreObj.receiveAccountName}}</span>
        </div>
        <div class="transfer-content" v-if="!successFlag">
            <div class="transfer-content-line1" style="margin-top: 3px;">
                <span class="transfer-content-line1-title">转账总额：</span><span class="transfer-content-line1-con" style="color: #FFA036;font-size: 18px;font-weight: 500;">{{transferPreObj.money}}元</span>
            </div>
            <div class="transfer-content-line1" style="margin-top: 19px;">
                <span class="transfer-content-line1-title">到账时间：</span><span class="transfer-content-line1-con">预计2小时内到账</span>
            </div>
            <div class="transfer-line-line"></div>
            <div class="transfer-content-line2">
                <a-icon type="exclamation-circle" style="color: #FFA036;margin:0 4px 0 0;font-size: 14px;" />付款后，资金将自动进入对方账户，无法撤回或退款，请谨慎操作！
            </div>
            <div class="verifyTitle">
              请验证您的身份
            </div>
            <div class="warningStle">
              短信验证码将发送至绑定手机：{{transferPreObj.ownerPhone|formatPhone}}
            </div>
            <div class="verifyCodeStyle">
            ...
            <div class="transitionStyle" v-if="transferPreObj.accountUserType==1&&transferPreObj.transactionVerifyMode!=9" >
            ...
            <div class="line4">
                ...
                <a-button size="large" style="width: 160px;" type="primary" class="formItemMarginButton saveFont" :loading="saveLoading" @click="transfer">
                    确认信息并付款
                </a-button>
            </div>
        </div>
        <div class="successStyle" v-if="successFlag">
            ...
            <div class="successRadiusStyle1">转帐受理成功</div>
            <div class="successRadiusStyle2">预计2小时内到达对方账户</div>
            <div class="successRadiusStyle3 cursor" @click="successClose">关闭</div>
          <div style="height: 50px;"></div>
        </div>
        <!-- 通商云支付-->
        <a-modal destroyOnClose centered v-model="htmlPannelDialog" :footer="null" class="ownModalBorder titDialog" :maskClosable="false" title="支付确认" :bodyStyle="{padding:'0px 0px 0px 0px'}" @cancel="cancelTransfer" width="1250px">
        ...
        <a-modal centered v-model="noSetPwdWarningDialog" :footer="null" class="ownModalBorder" title="" :bodyStyle="{padding:'44px 0px 20px 20px'}" @cancel="noSetPwdWarningDialog=false" width="432px">
          <no-set-pwd-warning :isOwner="transferPreObj.isOwner" :accountId="transferPreObj.accountId" @setSuccess="setPwdSuccess" @closeThis="noSetPwdWarningDialog=false"></no-set-pwd-warning>
        </a-modal>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        created() {
          this.isSetPayPwd = this.transferPreObj.isSetPayPwd;
        },
        methods: {
          cancelTransfer(){
            let _this = this;
          ...
          goAllinpayTransfer(){
            ...
            if(this.url){
              ...
              //去通商云转账
              let parameter = {
                accountId:this.transferPreObj.accountId,
                amount:Math.round(this.transferPreObj.money*100),
                receiveAccountId:this.transferPreObj.receiveAccountId,
                transactionVerifyMethod:2,
                source:1,
              }
              API.bizTransferApply(parameter).then(res=>{
                if(res.code === '200'){
          ...
          getVerifyCode(){
            if(this.canGetVerification) {
              ...
              if(this.transactionFlowId){
                ...
                let parameter = {
                  ...
                  amount:Math.round(this.transferPreObj.money*100),
                  receiveAccountId:this.transferPreObj.receiveAccountId,
                  transactionVerifyMethod:this.transferPreObj.transactionVerifyMode,
                  source:1,
                }
                API.bizTransferApply(parameter).then(res=>{
                  if(res.code === '200'){
            ...
            transfer(){
                ...
                //来自强检验打开通商云的返回
                if(this.transferPreObj.transactionVerifyMode&&this.transferPreObj.transactionVerifyMode==9&&this.isSetPayPwd==0){
                    this.noSetPwdWarningDialog = true;
                    //去通商云设置密码
                    return ;
                }
                if(this.transferPreObj.transactionVerifyMode&&this.transferPreObj.transactionVerifyMode==9&&this.url){
                  this.htmlPannelDialog = true;
                  return ;
                }
                ...
                API.transactionConfirm(parameter).then(res=>{
                    ...
                    if(res.code === '200'){
                        if(this.transferPreObj.transactionVerifyMode&&this.transferPreObj.transactionVerifyMode==9){
                            this.saveLoading = false;
                            this.url = res.data.transactionJumpUrl;
                            this.setPwdTransactionFlowId = res.data.transactionFlowId;
                            if(this.url){
