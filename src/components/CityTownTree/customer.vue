<template>
    <div @click.stop="showTree=true" class="u-bottom-border tree-wrap">
        ...
        <div v-show="showTree" @click.stop="showTree=true" class="tree" :style="{top: placementTop,bottom: placementBottom,left: placementLeft,right: placementRight}">
            ...
            selectName() {
                if (this.selectObj.name != this.selectName) {
                    if (this.selectName) {
                        ...
                        let parameter = {
                          fuzzyKeyword:this.selectName,
                          needCity:1,
                          needArea:this.showArea?1:0,
                          needTown:this.showTown?1:0,
                        }
                        PublicAPI.searchRegionInfoByKeyword(parameter).then(res => {
                            if (res.code == 200) {
                    ...
                    if (res.code == 200) {
                        ...
                        if(this.defaultCityId){
                          this.cityList.forEach((item,index)=>{
                            if(item.id == this.defaultCityId){
                              this.selectCityIndex = index;
                            }
                            this.getAreaData(this.defaultCityId);
                          })
                        }
                    }
                })
            ...
            selectAreaChange(item, index) {
                ...
                if (index) {
                    ...
                    this.selectObj = {
                        ...
                        areaId: item.id,
                        townId: '',
                        cityName: this.cityList[this.selectCityIndex].name,
                      areaName: item.name,
                      townName: '',
                      name:this.cityList[this.selectCityIndex].name+'/'+item.name,
                    }
                } else {
                    this.selectObj = {
                        cityId: this.cityList[this.selectCityIndex].id,
                ...
                if (index) {
                    this.selectObj = {
                      ...
                      areaId: this.areaList[this.selectAreaIndex].id,
                        townId: item.id,
                      cityName: this.cityList[this.selectCityIndex].name,
                      areaName:this.areaList[this.selectAreaIndex].name,
                      townName: item.name,
                      name:this.cityList[this.selectCityIndex].name+'/'+this.areaList[this.selectAreaIndex].name+'/'+item.name,
                    }
                } else {
                    this.selectObj = {
                      cityId: this.cityList[this.selectCityIndex].id,
                      ...
                      areaName:this.areaList[this.selectAreaIndex].name,
                      townName:'',
                      name:this.cityList[this.selectCityIndex].name+this.areaList[this.selectAreaIndex].name,
                    }
                }
            },
