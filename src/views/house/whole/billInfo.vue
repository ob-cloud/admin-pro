            ...
            <a-table rowKey="id" :bordered="true" :customRow="openDetail" :pagination="false" :columns="tableColumns" :dataSource="info">
                    <div slot="billStatus" slot-scope="text,record">
                        <div v-if="text==1" class="fontNormal">
                            <div class="IconRadius unDealBg f-fl"></div>
                            <div>
                                <span>未完成 </span>
                                <span style="color:#FB4246;font-size:12px;" v-if="record.overdueDays>0">逾{{record.overdueDays}}天</span>
                            </div>
                        </div>
                        <div v-else-if="text==2" class="fontProhibit fontNormal">
                            <div class="prohibitBg2 IconRadius normalBg f-fl"></div>
                            <div>
                                <span>已完成</span>
                            </div>
                        </div>
                    ...
                    <div slot="payerName" slot-scope="text,record">
                        <div class="f-fl" type="primary">
                            <p style="line-height:22px;">
                                <span v-if="record.bizType==2&&!permission.renterViewPhoneFlag">{{record.payerName}}-{{record.payerPhone|formatPhone}} </span>
                              <span v-else-if="record.bizType==3&&!permission.ownerViewPhoneFlag">{{record.payerName}}-{{record.payerPhone|formatPhone}} </span>
                              <span v-else>{{record.payerName}}-{{record.payerPhone}}</span>
                            </p>
                        </div>
                    ...
                    <div slot="predictTime" slot-scope="text,record">
                      ...
                      <span v-if="record.billStatus == 2">{{record.finishTime?$moment(record.finishTime).format('YYYY.MM.DD'):'-'}}</span>
                      <span v-else>{{text?$moment(text).format('YYYY.MM.DD'):'-'}}</span>
                    </div>
                    <div slot="beginTime" slot-scope="text,record">
                        {{$moment(text).format('YYYY.MM.DD')}} - {{$moment(record.endTime).format('YYYY.MM.DD')}}
                    </div>
...
<script>
import { billSimpleList } from '@/api/wholeHouse';
export default {
    name: "billInfo",
    props: {
        houseId: {
      ...
      getPermission(){
        JSON.parse(sessionStorage.getItem('permissions')).forEach(value => {
          if (value.mark == "mk_contract_tenant_zkdh"){
            this.permission.renterViewPhoneFlag = true;
          }
          if(value.mark == "mk_contract_owner_yzdh"){
            this.permission.ownerViewPhoneFlag = true;
          }
        });
      },
        ...
        getInfo(){
            billSimpleList(this.houseId,this.housePid,this.pagination.current,this.pagination.pageSize).then(res => {
                if (res.code === '200') {
...
.billInfo-box {
    ...
    .table {
        ...
        .fontNormal {
            .unDealBg{
                background-color: rgb(254, 60, 56);
            }
            .normalBg {
